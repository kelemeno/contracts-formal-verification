...
def ACond_for_6430914214554261284 (s₀ : State) : Literal := fromBool (s₀["src"]!! < s₀["srcEnd"]!!)
def APost_for_6430914214554261284 (s₀ s₉ : State) : Prop := for_6430914214554261284_post_concrete_of_code.1 s₀ s₉
def ABody_for_6430914214554261284 (s₀ s₉ : State) : Prop := for_6430914214554261284_body_concrete_of_code.1 s₀ s₉
def AFor_for_6430914214554261284 (s₀ s₉ : State) : Prop := True
...
lemma for_6430914214554261284_cond_abs_of_code {s₀ fuel} : eval fuel for_6430914214554261284_cond (s₀) = (s₀, ACond_for_6430914214554261284 (s₀)) := by
  ...
  simp [for_6430914214554261284_cond, evalArgs, head', reverse', Var', Lit', PrimCall', evalPrimCall]
  rw [EVMLt']
  simp [fromBool]
...
lemma for_6430914214554261284_concrete_of_post_abs {s₀ s₉ : State} :
  ...
  intro h
  simpa [APost_for_6430914214554261284] using h
...
lemma for_6430914214554261284_concrete_of_body_abs {s₀ s₉ : State} :
  ...
  intro h
  simpa [ABody_for_6430914214554261284] using h

lemma AZero_for_6430914214554261284 : ∀ s₀, isOk s₀ → ACond_for_6430914214554261284 (👌 s₀) = 0 → AFor_for_6430914214554261284 s₀ s₀ := by
  intro s₀ _ _
  trivial
...
:= by
  ...
  trivial
lemma AContinue_for_6430914214554261284 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isContinue s₂ → ¬ ACond_for_6430914214554261284 s₀ = 0 → ABody_for_6430914214554261284 s₀ s₂ → Spec APost_for_6430914214554261284 (🧟s₂) s₄ → Spec AFor_for_6430914214554261284 s₄ s₅ → AFor_for_6430914214554261284 s₀ s₅ := by
  intro s₀ s₂ s₄ s₅ _ _ _ _ _ _
  trivial
lemma ABreak_for_6430914214554261284 : ∀ s₀ s₂, isOk s₀ → isBreak s₂ → ¬ ACond_for_6430914214554261284 s₀ = 0 → ABody_for_6430914214554261284 s₀ s₂ → AFor_for_6430914214554261284 s₀ (🧟s₂) := by
  intro s₀ s₂ _ _ _ _
  trivial
lemma ALeave_for_6430914214554261284 : ∀ s₀ s₂, isOk s₀ → isLeave s₂ → ¬ ACond_for_6430914214554261284 s₀ = 0 → ABody_for_6430914214554261284 s₀ s₂ → AFor_for_6430914214554261284 s₀ s₂ := by
  intro s₀ s₂ _ _ _ _
  trivial
