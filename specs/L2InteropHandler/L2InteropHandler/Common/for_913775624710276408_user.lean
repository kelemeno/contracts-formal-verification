...
def ACond_for_913775624710276408 (s₀ : State) : Literal :=
  fromBool (s₀["src"]!! < s₀["srcEnd"]!!)

/-- Loop post: `src := add(src, 32)`. -/
def APost_for_913775624710276408 (s₀ s₉ : State) : Prop :=
...
and post. -/
def AFor_for_913775624710276408 (s₀ s₉ : State) : Prop :=
...
/-- Loop body: copy one calldata word to memory and advance the destination.
    ...
    let split_expr_40 := calldataload(src);  mstore(dst, split_expr_40);  dst := add(dst, 32) -/
def ABody_for_913775624710276408 (s₀ s₉ : State) : Prop :=
  ∀ evm store, s₀ = Ok evm store →
    let s₁ := (Ok evm store)⟦"split_expr_40" ↦
                EVMState.calldataload (Ok evm store).evm ((Ok evm store)["src"]!!)⟧
    let s₂ := s₁🇪⟦EVMState.mstore (Ok evm store).evm (s₁["dst"]!!) (s₁["split_expr_40"]!!)⟧
    s₉ = s₂⟦"dst" ↦ (s₂["dst"]!! + 32)⟧
...
lemma for_913775624710276408_concrete_of_post_abs {s₀ s₉ : State} :
  ...
  unfold for_913775624710276408_post_concrete_of_code APost_for_913775624710276408
...
lemma AZero_for_913775624710276408 : ∀ s₀, isOk s₀ → ACond_for_913775624710276408 (👌 s₀) = 0 → AFor_for_913775624710276408 s₀ s₀ := by
  intro s₀ hok hcond
  unfold AFor_for_913775624710276408 ACond_for_913775624710276408 at *
  intro evm store hs
  subst hs
  intro hlt
...
lemma AOk_for_913775624710276408 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isOk s₂ → ¬ ❓ s₅ → ¬ ACond_for_913775624710276408 s₀ = 0 → ABody_for_913775624710276408 s₀ s₂ → APost_for_913775624710276408 s₂ s₄ → Spec AFor_for_913775624710276408 s₄ s₅ → AFor_for_913775624710276408 s₀ s₅ := by
  ...
  · have h4 : s₄ = (Ok e2 st2)⟦"src" ↦ ((Ok e2 st2)["src"]!! + 32)⟧ := hpost e2 st2 rfl
    ...
    exact Spec_ok_unfold (P := AFor_for_913775624710276408) (s := s₄) (s' := s₅) hok4 h5 hspec
...
lemma AContinue_for_913775624710276408 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isContinue s₂ → ¬ ACond_for_913775624710276408 s₀ = 0 → ABody_for_913775624710276408 s₀ s₂ → Spec APost_for_913775624710276408 (🧟s₂) s₄ → Spec AFor_for_913775624710276408 s₄ s₅ → AFor_for_913775624710276408 s₀ s₅ := by
  ...
  · exact Spec_ok_unfold (P := AFor_for_913775624710276408) (s := Ok e4 st4) (s' := s₅) (by simp [isOk]) h5 hspec evm store hs
...
lemma for_913775624710276408_concrete_of_body_abs {s₀ s₉ : State} :
  ...
  unfold for_913775624710276408_body_concrete_of_code ABody_for_913775624710276408
  rcases s₀ with ⟨evm, store⟩ | _ | _ <;> [skip; aesop_spec; aesop_spec]
  apply spec_eq
  intro _hne hc
  intro evmA storeA hok
