...
/-- Loop condition: `lt(i, length)`. -/
def ACond_for_8976315411785817192 (s₀ : State) : Literal :=
  fromBool (s₀["i"]!! < s₀["length"]!!)

/-- Loop post: `i := add(i, 1)`. -/
def APost_for_8976315411785817192 (s₀ s₉ : State) : Prop :=
...
and post. -/
def AFor_for_8976315411785817192 (s₀ s₉ : State) : Prop :=
  ∀ evm store, s₉ = Ok evm store → ¬ ((Ok evm store)["i"]!! < (Ok evm store)["length"]!!)

/-- Loop body: copy one word from `srcPtr` to `pos`, advancing both cursors.

    let split_expr_16 := mload(srcPtr);  mstore(pos, split_expr_16)
    pos := add(pos, 32);  srcPtr := add(srcPtr, 32) -/
def ABody_for_8976315411785817192 (s₀ s₉ : State) : Prop :=
  ∀ evm store, s₀ = Ok evm store →
    let s₁ := (Ok evm store)⟦"split_expr_16" ↦
                EVMState.mload (Ok evm store).evm ((Ok evm store)["srcPtr"]!!)⟧
    let s₂ := s₁🇪⟦EVMState.mstore (Ok evm store).evm (s₁["pos"]!!) (s₁["split_expr_16"]!!)⟧
...
lemma for_8976315411785817192_cond_abs_of_code {s₀ fuel} : eval fuel for_8976315411785817192_cond (s₀) = (s₀, ACond_for_8976315411785817192 (s₀)) := by
  ...
  simp [for_8976315411785817192_cond, evalArgs, head', reverse', Var', Lit', PrimCall', evalPrimCall]
  rw [EVMLt']
  simp [fromBool]
...
lemma for_8976315411785817192_concrete_of_post_abs {s₀ s₉ : State} :
  ...
  unfold for_8976315411785817192_post_concrete_of_code APost_for_8976315411785817192
...
lemma AZero_for_8976315411785817192 : ∀ s₀, isOk s₀ → ACond_for_8976315411785817192 (👌 s₀) = 0 → AFor_for_8976315411785817192 s₀ s₀ := by
  intro s₀ hok hcond
  unfold AFor_for_8976315411785817192 ACond_for_8976315411785817192 at *
  intro evm store hs
  subst hs
  intro hlt
...
lemma AOk_for_8976315411785817192 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isOk s₂ → ¬ ❓ s₅ → ¬ ACond_for_8976315411785817192 s₀ = 0 → ABody_for_8976315411785817192 s₀ s₂ → APost_for_8976315411785817192 s₂ s₄ → Spec AFor_for_8976315411785817192 s₄ s₅ → AFor_for_8976315411785817192 s₀ s₅ := by
  ...
  · have h4 : s₄ = (Ok e2 st2)⟦"i" ↦ ((Ok e2 st2)["i"]!! + 1)⟧ := hpost e2 st2 rfl
    ...
    exact Spec_ok_unfold (P := AFor_for_8976315411785817192) (s := s₄) (s' := s₅) hok4 h5 hspec
  · exact absurd h2 (by simp [isOk])
  · exact absurd h2 (by simp [isOk])

lemma AContinue_for_8976315411785817192 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isContinue s₂ → ¬ ACond_for_8976315411785817192 s₀ = 0 → ABody_for_8976315411785817192 s₀ s₂ → Spec APost_for_8976315411785817192 (🧟s₂) s₄ → Spec AFor_for_8976315411785817192 s₄ s₅ → AFor_for_8976315411785817192 s₀ s₅ := by
  ...
  · exact Spec_ok_unfold (P := AFor_for_8976315411785817192) (s := Ok e4 st4) (s' := s₅) (by simp [isOk]) h5 hspec evm store hs
...
lemma ALeave_for_8976315411785817192 : ∀ s₀ s₂, isOk s₀ → isLeave s₂ → ¬ ACond_for_8976315411785817192 s₀ = 0 → ABody_for_8976315411785817192 s₀ s₂ → AFor_for_8976315411785817192 s₀ s₂ := by
...
lemma for_8976315411785817192_concrete_of_body_abs {s₀ s₉ : State} :
  ...
  unfold for_8976315411785817192_body_concrete_of_code ABody_for_8976315411785817192
  rcases s₀ with ⟨evm, store⟩ | _ | _ <;> [skip; aesop_spec; aesop_spec]
  apply spec_eq
  intro _hne hc
  intro evmA storeA hok
...
lemma ABreak_for_8976315411785817192 : ∀ s₀ s₂, isOk s₀ → isBreak s₂ → ¬ ACond_for_8976315411785817192 s₀ = 0 → ABody_for_8976315411785817192 s₀ s₂ → AFor_for_8976315411785817192 s₀ (🧟s₂) := by
