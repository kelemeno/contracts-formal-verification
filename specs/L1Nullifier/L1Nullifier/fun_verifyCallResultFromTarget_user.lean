...
def A_fun_verifyCallResultFromTarget (var_mpos : Identifier) (var_target var_success var_returndata_mpos var_errorMessage_mpos : Literal) (s₀ s₉ : State) : Prop := fun_verifyCallResultFromTarget_concrete_of_code.1 var_mpos var_target var_success var_returndata_mpos var_errorMessage_mpos s₀ s₉
