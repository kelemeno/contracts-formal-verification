...
def A_abi_decode_enum_TxStatus (value : Identifier) (offset : Literal) (s₀ s₉ : State) : Prop := abi_decode_enum_TxStatus_concrete_of_code.1 value offset s₀ s₉
...
lemma abi_decode_enum_TxStatus_abs_of_concrete {s₀ s₉ : State} {value offset} :
  ...
  intro h
  simpa [A_abi_decode_enum_TxStatus] using h
