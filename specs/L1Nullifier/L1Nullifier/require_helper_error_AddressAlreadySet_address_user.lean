...
def A_require_helper_error_AddressAlreadySet_address  (condition expr : Literal) (s₀ s₉ : State) : Prop := require_helper_error_AddressAlreadySet_address_concrete_of_code.1  condition expr s₀ s₉
...
lemma require_helper_error_AddressAlreadySet_address_abs_of_concrete {s₀ s₉ : State} { condition expr} :
  ...
  intro h
  simpa [A_require_helper_error_AddressAlreadySet_address] using h
