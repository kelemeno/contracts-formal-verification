...
def A_read_from_storage_split_offset_bool (value : Identifier) (slot : Literal) (s₀ s₉ : State) : Prop := read_from_storage_split_offset_bool_concrete_of_code.1 value slot s₀ s₉
...
lemma read_from_storage_split_offset_bool_abs_of_concrete {s₀ s₉ : State} {value slot} :
  ...
  intro h
  simpa [A_read_from_storage_split_offset_bool] using h
