...
def A_abi_decode_available_length_bytes (array : Identifier) (src length end_clear_sanitised_hrafn : Literal) (s₀ s₉ : State) : Prop := sorry
...
lemma abi_decode_available_length_bytes_abs_of_concrete {s₀ s₉ : State} {array src length end_clear_sanitised_hrafn} :
  ...
  unfold abi_decode_available_length_bytes_concrete_of_code A_abi_decode_available_length_bytes
  sorry
