...
def A_require_helper_error_ZeroAddress  (condition : Literal) (s₀ s₉ : State) : Prop := sorry
...
lemma require_helper_error_ZeroAddress_abs_of_concrete {s₀ s₉ : State} { condition} :
  ...
  unfold require_helper_error_ZeroAddress_concrete_of_code A_require_helper_error_ZeroAddress
  sorry
