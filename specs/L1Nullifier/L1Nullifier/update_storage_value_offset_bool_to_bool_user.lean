...
/-- Sets storage slot 0, keeping all bits except bits 8-15 (clears byte at offset 8, writes true=1 shifted by 8). -/
def A_update_storage_value_offset_bool_to_bool  (s₀ s₉ : State) : Prop :=
  s₉ = s₀🇪⟦s₀.evm.sstore 0 (Fin.lor (Fin.land (s₀.evm.sload 0) (UInt256.lnot 65280)) 256)⟧
...
lemma update_storage_value_offset_bool_to_bool_abs_of_concrete {s₀ s₉ : State}  :
  ...
  unfold update_storage_value_offset_bool_to_bool_concrete_of_code A_update_storage_value_offset_bool_to_bool
