...
def A_update_storage_value_offset_bool_to_bool   (s₀ s₉ : State) : Prop := update_storage_value_offset_bool_to_bool_concrete_of_code.1 s₀ s₉
...
lemma update_storage_value_offset_bool_to_bool_abs_of_concrete {s₀ s₉ : State}  :
  ...
  intro h
  simpa [A_update_storage_value_offset_bool_to_bool] using h
