...
def A_require_helper_stringliteral_e11a  (condition : Literal) (s₀ s₉ : State) : Prop := require_helper_stringliteral_e11a_concrete_of_code.1  condition s₀ s₉
...
lemma require_helper_stringliteral_e11a_abs_of_concrete {s₀ s₉ : State} { condition} :
  ...
  intro h
  simpa [A_require_helper_stringliteral_e11a] using h
