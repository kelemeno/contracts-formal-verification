...
def A_validator_assert_enum_TxStatus  (value : Literal) (s₀ s₉ : State) : Prop := sorry
...
lemma validator_assert_enum_TxStatus_abs_of_concrete {s₀ s₉ : State} { value} :
  ...
  unfold validator_assert_enum_TxStatus_concrete_of_code A_validator_assert_enum_TxStatus
  sorry
