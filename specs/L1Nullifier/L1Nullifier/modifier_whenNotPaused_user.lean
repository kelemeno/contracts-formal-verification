...
def A_modifier_whenNotPaused  (var__confirmTransferResultData_mpos : Literal) (s₀ s₉ : State) : Prop :=
  modifier_whenNotPaused_concrete_of_code.1 var__confirmTransferResultData_mpos s₀ s₉
...
lemma modifier_whenNotPaused_abs_of_concrete {s₀ s₉ : State} { var__confirmTransferResultData_mpos} :
  ...
  intro h
  simpa [A_modifier_whenNotPaused] using h
