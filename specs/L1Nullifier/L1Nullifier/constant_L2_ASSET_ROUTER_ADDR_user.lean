...
def A_constant_L2_ASSET_ROUTER_ADDR (ret : Identifier)  (s₀ s₉ : State) : Prop := constant_L2_ASSET_ROUTER_ADDR_concrete_of_code.1 ret s₀ s₉
...
lemma constant_L2_ASSET_ROUTER_ADDR_abs_of_concrete {s₀ s₉ : State} {ret } :
  ...
  intro h
  simpa [A_constant_L2_ASSET_ROUTER_ADDR] using h
