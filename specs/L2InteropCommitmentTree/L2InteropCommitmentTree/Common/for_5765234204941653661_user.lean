...
def levelGuardStateDyn (s₀ : State) : State :=
...
def APost_for_5765234204941653661 (s₀ s₉ : State) : Prop :=
...
i.e. the level index reached the stored count — see `levelGuardStateDyn_flag_iff` for
...
def AFor_for_5765234204941653661 (s₀ s₉ : State) : Prop :=
  ∀ evm store, s₉ = Ok evm store →
...
def ABody_for_5765234204941653661 (s₀ s₉ : State) : Prop :=
  ∃ s₁, Spec A_if_4006823798342809328 (levelGuardStateDyn s₀) s₁ ∧
    ∃ s₂, Spec A_if_8218475617004033221 s₁ s₂ ∧
      ∃ s₃, Spec A_block_3711784909988835814 s₂ s₃ ∧
...
lemma levelGuardStateDyn_flag_iff {s : State} (hok : isOk s) :
    (levelGuardStateDyn s)["split_expr_6"]!! = 0 ↔
      ¬ (s["var_i"]!! < Clear.EVMState.sload s.evm 0) := by
  unfold levelGuardStateDyn
...
lemma for_5765234204941653661_concrete_of_post_abs {s₀ s₉ : State} :
  ...
  unfold for_5765234204941653661_post_concrete_of_code APost_for_5765234204941653661
...
lemma for_5765234204941653661_concrete_of_body_abs {s₀ s₉ : State} :
  ...
  unfold for_5765234204941653661_body_concrete_of_code ABody_for_5765234204941653661 levelGuardStateDyn
  rcases s₀ with ⟨evm, store⟩ | _ | _ <;> [skip; aesop_spec; aesop_spec]
  apply spec_eq
  intro _hne hc
...
lemma AZero_for_5765234204941653661 : ∀ s₀, isOk s₀ → ACond_for_5765234204941653661 (👌 s₀) = 0 → AFor_for_5765234204941653661 s₀ s₀ := by
  intro s₀ _hok hcond
  unfold ACond_for_5765234204941653661 at hcond
  exact absurd hcond (by decide)

lemma AOk_for_5765234204941653661 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isOk s₂ → ¬ ❓ s₅ → ¬ ACond_for_5765234204941653661 s₀ = 0 → ABody_for_5765234204941653661 s₀ s₂ → APost_for_5765234204941653661 s₂ s₄ → Spec AFor_for_5765234204941653661 s₄ s₅ → AFor_for_5765234204941653661 s₀ s₅ := by
  ...
  · have h4 : s₄ = (Ok e2 st2)⟦"var_i" ↦ ((Ok e2 st2)["var_i"]!! + 1)⟧ := hpost e2 st2 rfl
    ...
    exact Spec_ok_unfold (P := AFor_for_5765234204941653661) (s := s₄) (s' := s₅) hok4 h5 hspec
...
lemma AContinue_for_5765234204941653661 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isContinue s₂ → ¬ ACond_for_5765234204941653661 s₀ = 0 → ABody_for_5765234204941653661 s₀ s₂ → Spec APost_for_5765234204941653661 (🧟s₂) s₄ → Spec AFor_for_5765234204941653661 s₄ s₅ → AFor_for_5765234204941653661 s₀ s₅ := by
  ...
  · exact Spec_ok_unfold (P := AFor_for_5765234204941653661) (by simp [isOk]) h5 hspec evm store hs
...
lemma ABreak_for_5765234204941653661 : ∀ s₀ s₂, isOk s₀ → isBreak s₂ → ¬ ACond_for_5765234204941653661 s₀ = 0 → ABody_for_5765234204941653661 s₀ s₂ → AFor_for_5765234204941653661 s₀ (🧟s₂) := by
  ...
  have hgc : isOk (levelGuardStateDyn s₀) := by
    unfold levelGuardStateDyn; simp only [isOk_insert]; exact h0
  have hg1 := Spec_ok_unfold hgc h1nf h₁
  by_cases hf1 : (levelGuardStateDyn s₀)["split_expr_6"]!! = 0
  · have e1 : s₁ = 💔(levelGuardStateDyn s₀) := hg1.1 hf1
    ...
    rw [← hs]
    exact hf1
  · have e1 : s₁ = levelGuardStateDyn s₀ := hg1.2 hf1
    ...
    · exfalso
      ...
      have hs3ok : isOk s₃ := block_3711784909988835814_isOk hs2ok h3nf (Spec_ok_unfold hs2ok h3nf h₃)
