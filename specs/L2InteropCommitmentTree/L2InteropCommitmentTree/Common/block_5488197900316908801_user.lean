...
def A_block_5488197900316908801 (s₀ s₉ : State) : Prop :=
  let a := s₀⟦"split_expr_3" ↦ s₀["expr_mpos"]!! + 96⟧
  let m1 := a🇪⟦Clear.EVMState.mstore s₀.evm (a["split_expr_3"]!!) (a["_3"]!!)⟧
  let m2 := m1🇪⟦Clear.EVMState.mstore m1.evm (m1["expr_mpos"]!!) 96⟧
  ∃ s, Spec (A_finalize_allocation (m2["expr_mpos"]!!) 128) m2 s ∧
    s₉ = s⟦"split_expr_4" ↦ Clear.EVMState.mload s.evm (s["expr_mpos"]!!)⟧
...
lemma block_5488197900316908801_abs_of_concrete {s₀ s₉ : State} :
  ...
  unfold block_5488197900316908801_concrete_of_code A_block_5488197900316908801
