...
import generated.L2InteropCommitmentTree.L2InteropCommitmentTree.Common.block_5648918763415424361_gen
...
def A_block_5648918763415424361 (s₀ s₉ : State) : Prop :=
  ∃ s₁, Spec (A_storage_array_index_access_bytes32_dyn_ptr "_5" "_6" 2 (s₀["var_i"]!!)) s₀ s₁ ∧
    ∃ s₂, Spec (A_checked_sub_uint256 "split_expr_6" (s₁["var_index"]!!)) s₁ s₂ ∧
      ∃ s₃, Spec (A_storage_array_index_access_bytes32_dyn_ptr "_7" "_8"
...
lemma block_5648918763415424361_abs_of_concrete {s₀ s₉ : State} :
  Spec block_5648918763415424361_concrete_of_code s₀ s₉ →
  Spec A_block_5648918763415424361 s₀ s₉ := by
  unfold block_5648918763415424361_concrete_of_code A_block_5648918763415424361
...
lemma block_5648918763415424361_isOk {s₀ s₉ : State} (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (h : A_block_5648918763415424361 s₀ s₉) : isOk s₉ := by
...
lemma block_5648918763415424361_not_break {s₀ s₉ : State} (hok : isOk s₀) (hnf : ¬ ❓ s₉)
...
lemma block_5648918763415424361_sload {q : UInt256} {s₀ s₉ : State} (hok : isOk s₀)
    (hnf : ¬ ❓ s₉) (h : A_block_5648918763415424361 s₀ s₉) :
...
lemma block_5648918763415424361_config {s₀ s₉ : State} (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (hR : RangeInWindow s₀.evm) (hC : CachedInWindow s₀.evm)
    (h : A_block_5648918763415424361 s₀ s₉) :
