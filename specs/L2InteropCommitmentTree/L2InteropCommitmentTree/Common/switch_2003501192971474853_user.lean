...
lemma switch_2003501192971474853_frame {v : Identifier} {s₀ s₉ : State}
    (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (hv : v ∉ (["_9", "_10", "split_expr_9", "_11", "_12", "split_expr_10", "_13", "_14",
      "split_expr_11", "expr"] : List Identifier))
    (h : A_switch_2003501192971474853 s₀ s₉) : s₉[v]!! = s₀[v]!! := by
  ...
  · rw [hbrA hc] at hnf ⊢
    ...
    have e5 : s₅[v]!! = s₀[v]!! :=
      storage_array_index_access_bytes32_dyn_ptr_frame hok h5nf h13 h14
  ...
  · rw [hbrB hc] at hnf ⊢
    ...
    have e3 : s₃[v]!! = s₂[v]!! :=
      storage_array_index_access_bytes32_dyn_ptr_frame hs2 h3nf h11 h12
        (Spec_ok_unfold hs2 h3nf h₃)
    have e2 : s₂[v]!! = s₁[v]!! :=
      checked_add_uint256_frame hs1 h2nf hadd (Spec_ok_unfold hs1 h2nf h₂)
    have e1 : s₁[v]!! = s₀[v]!! :=
      storage_array_index_access_bytes32_dyn_ptr_frame hok h1nf h9 h10
        (Spec_ok_unfold hok h1nf h₁)
    rw [e4, lookup_insert_of_ne hld, e3, e2, e1]


/-- **STORAGE FRAME.**  Both branches only read. -/
lemma switch_2003501192971474853_sload {q : UInt256} {s₀ s₉ : State} (hok : isOk s₀)
    (hnf : ¬ ❓ s₉) (h : A_switch_2003501192971474853 s₀ s₉) :
  ...
  · rw [hbrA hc] at hnf ⊢
    ...
    rw [storage_array_index_access_bytes32_dyn_ptr_sload hok h5nf
      (Spec_ok_unfold hok h5nf h₅)]
  · rw [hbrB hc] at hnf ⊢
    have h3nf : ¬ ❓ s₃ := by
      intro hoo
...
lemma switch_2003501192971474853_config {s₀ s₉ : State} (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (hR : RangeInWindow s₀.evm) (hC : CachedInWindow s₀.evm)
    (h : A_switch_2003501192971474853 s₀ s₉) :
  ...
  · rw [hbrA hc] at hnf ⊢
    ...
    have hs5 : isOk s₅ :=
      storage_array_index_access_bytes32_dyn_ptr_isOk h5nf (Spec_ok_unfold hok h5nf h₅)
    obtain ⟨hR5, hC5⟩ := storage_array_index_access_bytes32_dyn_ptr_config hok h5nf hR hC
      (Spec_ok_unfold hok h5nf h₅)
    have hld2ok : isOk (s₅⟦"split_expr_11" ↦ Clear.EVMState.sload s₅.evm (s₅["_13"]!!)⟧) :=
      isOk_insert.mpr hs5
    rw [extract_from_storage_value_dynamict_bytes32_evm hld2ok
