...
/-- **THE SLOT THIS BLOCK WRITES IS NEVER A LOW SLOT -- WITHOUT A FUEL BUDGET.**

Same conclusion as `_slot_not_low`, with the keccak side condition met by the flag on the
result rather than by two units counted in advance.  The written slot is the SECOND
accessor's, and this block's own output state is that accessor's output state, so the
caller's flag lands exactly where it is needed with no walking back at all.

The index bound stays: it is about the `+ index` after the hash, not about the hash. -/
lemma block_8439353917263816235_slot_not_low_of_clean {c : Literal} {s₀ s₉ : State}
    (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (hR : Clear.KeccakLowSlot.RangeInWindow s₀.evm)
    (hC : Clear.KeccakLowSlot.CachedInWindow s₀.evm)
    (hclean : Clear.KeccakClean.Clean s₉.evm)
  ...
  have hjb : (s₄["var_index"]!!).val < Clear.KeccakInjective.lowSlotBound := by
    rw [ei4, ei3, ei2, ei1]
    exact Clear.FinBits.shiftRight_one_lt_of_lt hj
  exact storage_array_index_access_bytes32_dyn_ptr_slot_not_low_of_clean hs4 hnf hR4 hC4
    hclean hjb hcl (Spec_ok_unfold hs4 hnf h₅)
