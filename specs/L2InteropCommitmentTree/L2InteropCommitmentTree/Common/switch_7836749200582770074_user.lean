...
lemma switch_7836749200582770074_config {s₀ s₉ : State} (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (hR : RangeInWindow s₀.evm) (hC : CachedInWindow s₀.evm)
    (h : A_switch_7836749200582770074 s₀ s₉) :
  ...
  · rw [hbrEven hc] at hnf ⊢
    ...
    have hs3 : isOk s₃ :=
      switch_8987501505216042171_isOk hins h3nf (Spec_ok_unfold hins h3nf h₃)
    obtain ⟨hR3, hC3⟩ := switch_8987501505216042171_config hins h3nf
  ...
  · rw [hbrOdd hc] at hnf ⊢
    ...
    have hs1 : isOk s₁ :=
      block_3834594906904189566_isOk hok h1nf (Spec_ok_unfold hok h1nf h₁)
    obtain ⟨hR1, hC1⟩ := block_3834594906904189566_config hok h1nf hR hC
