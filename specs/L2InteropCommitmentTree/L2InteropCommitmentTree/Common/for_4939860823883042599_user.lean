...
lemma ABreak_for_4939860823883042599 : ∀ s₀ s₂, isOk s₀ → isBreak s₂ → ¬ ACond_for_4939860823883042599 s₀ = 0 → ABody_for_4939860823883042599 s₀ s₂ → AFor_for_4939860823883042599 s₀ (🧟s₂) := by
  ...
  · have e1 : s₁ = 💔(rootGuardStateDyn s₀) := hg.1 hf
    ...
    · -- and what the flag MEANS: the level counter reached the stored level count
      intro _
      have hvi : (rootGuardStateDyn s₀)["var_i"]!! = s₀["var_i"]!! := by
        unfold rootGuardStateDyn
        rw [lookup_insert_of_ne (by decide), lookup_insert_of_ne (by decide)]
      have hevm : (rootGuardStateDyn s₀).evm = s₀.evm := by
        unfold rootGuardStateDyn; simp only [evm_insert]
      rw [hvi, hevm]
      exact (rootGuardStateDyn_flag_iff h0).mp hf
...
existentially: `w` is `s₄["_17"]`, an address computed inside the body, so a caller cannot
...
lemma ABody_for_4939860823883042599_writes_one_slot {s₀ s₉ : State} (hok : isOk s₀)
    (hok9 : isOk s₉) (h : ABody_for_4939860823883042599 s₀ s₉) :
  ...
  · have e1 : s₁ = rootGuardStateDyn s₀ := hg.2 hf
    ...
    have hs4 : isOk s₄ :=
      block_2743596091140315824_isOk hs3 h4nf (Spec_ok_unfold hs3 h4nf h₄)
    refine ⟨s₄["_17"]!!, fun q hq => ?_⟩
    rw [block_2896862189596047701_sload hs4 h5nf hq (Spec_ok_unfold hs4 h5nf h₅),
      block_2743596091140315824_sload hs3 h4nf (Spec_ok_unfold hs3 h4nf h₄),
      switch_7836749200582770074_sload hs2 h3nf (Spec_ok_unfold hs2 h3nf h₃),
...
lemma ABody_for_4939860823883042599_config {s₀ s₉ : State} (hok : isOk s₀) (hok9 : isOk s₉)
  ...
  have hgc : isOk (rootGuardStateDyn s₀) := by
    unfold rootGuardStateDyn; simp only [isOk_insert]; exact hok
  have hgce : (rootGuardStateDyn s₀).evm = s₀.evm := by
    unfold rootGuardStateDyn; simp only [evm_insert]
  have hg := Spec_ok_unfold hgc h1nf h₁
  by_cases hf : (rootGuardStateDyn s₀)["split_expr_4"]!! = 0
  · exfalso
  ...
  · have e1 : s₁ = rootGuardStateDyn s₀ := hg.2 hf
    ...
    obtain ⟨hR3, hC3⟩ := switch_7836749200582770074_config hs2 h3nf
      (by rw [e2, h1e]; exact hR) (by rw [e2, h1e]; exact hC) (Spec_ok_unfold hs2 h3nf h₃)
    obtain ⟨hR4, hC4⟩ := block_2743596091140315824_config hs3 h4nf hR3 hC3
