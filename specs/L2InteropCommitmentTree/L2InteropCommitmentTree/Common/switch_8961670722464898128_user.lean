...
union of the two. -/
lemma switch_8961670722464898128_frame {v : Identifier} {s₀ s₉ : State}
    (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    ...
    (h : A_switch_8961670722464898128 s₀ s₉) : s₉[v]!! = s₀[v]!! := by
  ...
  · -- even: the inner switch from `s₀⟦expr ↦ 0⟧`, then the fold hash
    ...
    have h3nf : ¬ ❓ s₃ := fun hoo => hnf (Clear.isOutOfFuel_of_Spec_of_isOutOfFuel h₄ hoo)
    have hs3 : isOk s₃ :=
      switch_2003501192971474853_isOk hins h3nf (Spec_ok_unfold hins h3nf h₃)
    have e4 : s₄[v]!! = s₃[v]!! :=
      fun_efficientHash_frame hs3 hnf hcur (Spec_ok_unfold hs3 hnf h₄)
    have e3 : s₃[v]!! = (s₀⟦"expr" ↦ 0⟧)[v]!! :=
      switch_2003501192971474853_frame hins h3nf hL2003 (Spec_ok_unfold hins h3nf h₃)
    rw [e4, e3, lookup_insert_of_ne hexpr]
  · -- odd: the sibling load, then its hash
    rw [hbrOdd hc] at hnf ⊢
    have h1nf : ¬ ❓ s₁ := fun hoo => hnf (Clear.isOutOfFuel_of_Spec_of_isOutOfFuel h₂ hoo)
    have hs1 : isOk s₁ :=
      block_5648918763415424361_isOk hok h1nf (Spec_ok_unfold hok h1nf h₁)
    have e2 : s₂[v]!! = s₁[v]!! :=
      block_1432253982873054235_frame hs1 hnf hcur (Spec_ok_unfold hs1 hnf h₂)
    have e1 : s₁[v]!! = s₀[v]!! :=
      block_5648918763415424361_frame hok h1nf hL5648 (Spec_ok_unfold hok h1nf h₁)
    rw [e2, e1]


/-- **STORAGE FRAME.**  The parity switch reads siblings and hashes; no slot changes. -/
lemma switch_8961670722464898128_sload {q : UInt256} {s₀ s₉ : State} (hok : isOk s₀)
    (hnf : ¬ ❓ s₉) (h : A_switch_8961670722464898128 s₀ s₉) :
  ...
  · rw [hbrEven hc] at hnf ⊢
    ...
    rw [fun_efficientHash_sload hs3 (Spec_ok_unfold hs3 hnf h₄),
      switch_2003501192971474853_sload hins h3nf (Spec_ok_unfold hins h3nf h₃)]
  ...
  · rw [hbrOdd hc] at hnf ⊢
    ...
    rw [block_1432253982873054235_sload hs1 hnf (Spec_ok_unfold hs1 hnf h₂),
      block_5648918763415424361_sload hok h1nf (Spec_ok_unfold hok h1nf h₁)]


/-- **CONFIG FRAME.**  The parity switch, both branches. -/
lemma switch_8961670722464898128_config {s₀ s₉ : State} (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (hR : RangeInWindow s₀.evm) (hC : CachedInWindow s₀.evm)
    (h : A_switch_8961670722464898128 s₀ s₉) :
  ...
  · rw [hbrEven hc] at hnf ⊢
    ...
    have hs3 : isOk s₃ :=
      switch_2003501192971474853_isOk hins h3nf (Spec_ok_unfold hins h3nf h₃)
    obtain ⟨hR3, hC3⟩ := switch_2003501192971474853_config hins h3nf
  ...
  · rw [hbrOdd hc] at hnf ⊢
    ...
    have hs1 : isOk s₁ :=
      block_5648918763415424361_isOk hok h1nf (Spec_ok_unfold hok h1nf h₁)
    obtain ⟨hR1, hC1⟩ := block_5648918763415424361_config hok h1nf hR hC
      (Spec_ok_unfold hok h1nf h₁)
    exact block_1432253982873054235_config hs1 hnf hR1 hC1 (Spec_ok_unfold hs1 hnf h₂)
