...
import generated.L2InteropCommitmentTree.L2InteropCommitmentTree.Common.for_4843491680166179088_gen
...
def ACond_for_4843491680166179088 (s₀ : State) : Literal := 1
/-- The state the break guard sees: the level count loaded and compared. -/
def rootGuardStateGenDyn (s₀ : State) : State :=
...
def APost_for_4843491680166179088 (s₀ s₉ : State) : Prop :=
...
def AFor_for_4843491680166179088 (s₀ s₉ : State) : Prop :=
...
def ABody_for_4843491680166179088 (s₀ s₉ : State) : Prop :=
  ∃ s₁, Spec A_if_6078234115189856909 (rootGuardStateGenDyn s₀) s₁ ∧
    ∃ s₂, Spec (A_mod_uint256 "split_expr_6" (s₁["var_index"]!!)) s₁ s₂ ∧
      ∃ s₃, Spec A_switch_1809072996261231762 s₂ s₃ ∧
        ∃ s₄, Spec A_block_2936313771097161809 s₃ s₄ ∧
          ∃ s₅, Spec A_block_7643149059429413085 s₄ s₅ ∧
            s₉ = s₅

/-- The flag is zero exactly when the level index has reached the stored count. -/
lemma rootGuardStateGenDyn_flag_iff {s : State} (hok : isOk s) :
    (rootGuardStateGenDyn s)["split_expr_5"]!! = 0 ↔
...
lemma for_4843491680166179088_cond_abs_of_code {s₀ fuel} : eval fuel for_4843491680166179088_cond (s₀) = (s₀, ACond_for_4843491680166179088 (s₀)) := by
  unfold eval ACond_for_4843491680166179088
  simp [for_4843491680166179088_cond, Lit']

lemma for_4843491680166179088_concrete_of_post_abs {s₀ s₉ : State} :
  Spec for_4843491680166179088_post_concrete_of_code s₀ s₉ →
  Spec APost_for_4843491680166179088 s₀ s₉ := by
  unfold for_4843491680166179088_post_concrete_of_code APost_for_4843491680166179088
...
lemma for_4843491680166179088_concrete_of_body_abs {s₀ s₉ : State} :
  Spec for_4843491680166179088_body_concrete_of_code s₀ s₉ →
  Spec ABody_for_4843491680166179088 s₀ s₉ := by
  unfold for_4843491680166179088_body_concrete_of_code ABody_for_4843491680166179088 rootGuardStateGenDyn
  rcases s₀ with ⟨evm, store⟩ | _ | _ <;> [skip; aesop_spec; aesop_spec]
  apply spec_eq
  intro _hne hc
...
lemma ABody_for_4843491680166179088_isOk_or_isBreak {s₀ s₉ : State} (hok : isOk s₀)
    (hnf : ¬ ❓ s₉) (h : ABody_for_4843491680166179088 s₀ s₉) : isOk s₉ ∨ isBreak s₉ := by
...
lemma ABody_for_4843491680166179088_var_i {s₀ s₉ : State} (hok : isOk s₀) (hok9 : isOk s₉)
    (h : ABody_for_4843491680166179088 s₀ s₉) : s₉["var_i"]!! = s₀["var_i"]!! := by
  obtain ⟨s₁, h₁, s₂, h₂, s₃, h₃, s₄, h₄, s₅, h₅, heq⟩ := h
  rw [heq] at hok9 ⊢
  ...
  · have e1 : s₁ = rootGuardStateGenDyn s₀ := hg.2 hf
    ...
    have e4 : s₄["var_i"]!! = s₃["var_i"]!! :=
      block_2936313771097161809_frame hs3 h4nf (by decide) (Spec_ok_unfold hs3 h4nf h₄)
    have e3 : s₃["var_i"]!! = s₂["var_i"]!! :=
      switch_1809072996261231762_frame hs2 h3nf (by decide) (Spec_ok_unfold hs2 h3nf h₃)
...
lemma ABody_for_4843491680166179088_index {s₀ s₉ : State} (hok : isOk s₀) (hok9 : isOk s₉)
    (h : ABody_for_4843491680166179088 s₀ s₉) :
  ...
  · have e1 : s₁ = rootGuardStateGenDyn s₀ := hg.2 hf
    ...
    have e4 : s₄["var_index"]!! = Fin.shiftRight (s₃["var_index"]!!) 1 :=
      block_2936313771097161809_index hs3 h4nf (Spec_ok_unfold hs3 h4nf h₄)
...
lemma AZero_for_4843491680166179088 : ∀ s₀, isOk s₀ → ACond_for_4843491680166179088 (👌 s₀) = 0 → AFor_for_4843491680166179088 s₀ s₀ := by
  intro s₀ _hok hcond
  unfold ACond_for_4843491680166179088 at hcond
  exact absurd hcond (by decide)

lemma AOk_for_4843491680166179088 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isOk s₂ → ¬ ❓ s₅ → ¬ ACond_for_4843491680166179088 s₀ = 0 → ABody_for_4843491680166179088 s₀ s₂ → APost_for_4843491680166179088 s₂ s₄ → Spec AFor_for_4843491680166179088 s₄ s₅ → AFor_for_4843491680166179088 s₀ s₅ := by
  ...
  · have hok2 : isOk (Ok e2 st2 : State) := by simp [isOk]
    ...
    have hfor : AFor_for_4843491680166179088 s₄ s₅ :=
      Spec_ok_unfold (P := AFor_for_4843491680166179088) hok4 h5 hspec
    ...
    have hbidx : (Ok e2 st2 : State)["var_index"]!! = Fin.shiftRight (s₀["var_index"]!!) 1 :=
      ABody_for_4843491680166179088_index h0 hok2 hbody
    have hbi : (Ok e2 st2 : State)["var_i"]!! = s₀["var_i"]!! :=
      ABody_for_4843491680166179088_var_i h0 hok2 hbody
...
lemma AContinue_for_4843491680166179088 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isContinue s₂ → ¬ ACond_for_4843491680166179088 s₀ = 0 → ABody_for_4843491680166179088 s₀ s₂ → Spec APost_for_4843491680166179088 (🧟s₂) s₄ → Spec AFor_for_4843491680166179088 s₄ s₅ → AFor_for_4843491680166179088 s₀ s₅ := by
  intro s₀ s₂ _s₄ _s₅ h0 h2 _hcond hbody _hpost _hspec
  exfalso
  rcases ABody_for_4843491680166179088_isOk_or_isBreak h0
...
lemma ABreak_for_4843491680166179088 : ∀ s₀ s₂, isOk s₀ → isBreak s₂ → ¬ ACond_for_4843491680166179088 s₀ = 0 → ABody_for_4843491680166179088 s₀ s₂ → AFor_for_4843491680166179088 s₀ (🧟s₂) := by
  ...
  have h1nf : ¬ ❓ s₁ := fun hoo => h2nf (Clear.isOutOfFuel_of_Spec_of_isOutOfFuel h₂ hoo)
  have hgc : isOk (rootGuardStateGenDyn s₀) := by
    unfold rootGuardStateGenDyn; simp only [isOk_insert]; exact h0
  have hg := Spec_ok_unfold hgc h1nf h₁
  by_cases hf : (rootGuardStateGenDyn s₀)["split_expr_5"]!! = 0
  · have e1 : s₁ = 💔(rootGuardStateGenDyn s₀) := hg.1 hf
    have hb1 : isBreak s₁ := by rw [e1]; exact Clear.isBreak_setBreak hgc
    ...
    · -- and what the flag MEANS: the level counter reached the stored level count
      intro _
      have hvi : (rootGuardStateGenDyn s₀)["var_i"]!! = s₀["var_i"]!! := by
        unfold rootGuardStateGenDyn
        rw [lookup_insert_of_ne (by decide), lookup_insert_of_ne (by decide)]
      have hevm : (rootGuardStateGenDyn s₀).evm = s₀.evm := by
        unfold rootGuardStateGenDyn; simp only [evm_insert]
      have hvs : (rootGuardStateGenDyn s₀)["var_self_slot"]!! = s₀["var_self_slot"]!! := by
        unfold rootGuardStateGenDyn
        rw [lookup_insert_of_ne (by decide), lookup_insert_of_ne (by decide)]
      rw [hvi, hevm, hvs]
      exact (rootGuardStateGenDyn_flag_iff h0).mp hf
  ...
  · exfalso
    ...
    have hs1 : isOk s₁ := by rw [e1]; exact hgc
    have hs2 : isOk s₂ := mod_uint256_isOk hs1 (Spec_ok_unfold hs1 h2nf h₂)
    have hs3 : isOk s₃ := switch_1809072996261231762_isOk hs2 h3nf (Spec_ok_unfold hs2 h3nf h₃)
    have hs4 : isOk s₄ := block_2936313771097161809_isOk hs3 h4nf (Spec_ok_unfold hs3 h4nf h₄)
...
lemma ALeave_for_4843491680166179088 : ∀ s₀ s₂, isOk s₀ → isLeave s₂ → ¬ ACond_for_4843491680166179088 s₀ = 0 → ABody_for_4843491680166179088 s₀ s₂ → AFor_for_4843491680166179088 s₀ s₂ := by
...
lemma ABody_for_4843491680166179088_config {s₀ s₉ : State} (hok : isOk s₀) (hok9 : isOk s₉)
  ...
  have hgc : isOk (rootGuardStateGenDyn s₀) := by
    unfold rootGuardStateGenDyn; simp only [isOk_insert]; exact hok
  have hgce : (rootGuardStateGenDyn s₀).evm = s₀.evm := by
    unfold rootGuardStateGenDyn; simp only [evm_insert]
  have hg := Spec_ok_unfold hgc h1nf h₁
  by_cases hf : (rootGuardStateGenDyn s₀)["split_expr_5"]!! = 0
  · exfalso
  ...
  · have e1 : s₁ = rootGuardStateGenDyn s₀ := hg.2 hf
    ...
    obtain ⟨hR3, hC3⟩ := switch_1809072996261231762_config hs2 h3nf
      (by rw [e2, h1e]; exact hR) (by rw [e2, h1e]; exact hC) (Spec_ok_unfold hs2 h3nf h₃)
    obtain ⟨hR4, hC4⟩ := block_2936313771097161809_config hs3 h4nf hR3 hC3
