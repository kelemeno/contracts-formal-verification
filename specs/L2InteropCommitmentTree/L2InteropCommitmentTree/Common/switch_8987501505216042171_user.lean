...
import generated.L2InteropCommitmentTree.L2InteropCommitmentTree.Common.switch_8987501505216042171_gen
...
taken branch. -/
def A_switch_8987501505216042171 (s₀ s₉ : State) : Prop :=
  ∃ s₁, Spec (A_storage_array_index_access_bytes32_dyn__dyn "_9" "_10" 2 (s₀["var_i"]!!)) s₀ s₁ ∧
    ∃ s₂, Spec (A_checked_add_uint256 "split_expr_9" (s₁["var_index"]!!)) s₁ s₂ ∧
      ∃ s₃, Spec (A_storage_array_index_access_bytes32_dyn__dyn "_11" "_12" (s₂["_9"]!!) (s₂["split_expr_9"]!!)) s₂ s₃ ∧
        (let ld := s₃⟦"split_expr_10" ↦ Clear.EVMState.sload s₃.evm (s₃["_11"]!!)⟧
         ∃ s₄, Spec (A_extract_from_storage_value_dynamict_bytes32 "expr" (ld["split_expr_10"]!!) (ld["_12"]!!)) ld s₄ ∧
           ∃ s₅, Spec (A_storage_array_index_access_bytes32_dyn__dyn "_13" "_14" 3 (s₀["var_i"]!!)) s₀ s₅ ∧
...
lemma switch_8987501505216042171_abs_of_concrete {s₀ s₉ : State} :
  Spec switch_8987501505216042171_concrete_of_code s₀ s₉ →
  Spec A_switch_8987501505216042171 s₀ s₉ := by
  unfold switch_8987501505216042171_concrete_of_code A_switch_8987501505216042171
...
lemma switch_8987501505216042171_uses_default {s₀ s₉ : State} (h : A_switch_8987501505216042171 s₀ s₉)
    (hedge : s₀["var_maxNodeNumber"]!! = s₀["var_index"]!!) :
    ∃ s₅, Spec (A_storage_array_index_access_bytes32_dyn__dyn "_13" "_14" 3 (s₀["var_i"]!!)) s₀ s₅ := by
...
lemma switch_8987501505216042171_isOk {s₀ s₉ : State} (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (h : A_switch_8987501505216042171 s₀ s₉) : isOk s₉ := by
  ...
  · rw [hedge he] at hnf ⊢
    ...
    have hs5 : isOk s₅ :=
      storage_array_index_access_bytes32_dyn__dyn_isOk h5nf (Spec_ok_unfold hok h5nf h₅)
...
lemma switch_8987501505216042171_not_break {s₀ s₉ : State} (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (h : A_switch_8987501505216042171 s₀ s₉) : ¬ isBreak s₉ :=
  fun hb => not_isOk_of_isBreak hb (switch_8987501505216042171_isOk hok hnf h)


/-- **FRAME.**  Both branches write only their own temporaries.

Note the two chains start from `s₀` INDEPENDENTLY -- the zero-hash branch (`s₅`, array 3)
does not run after the sibling branch -- so each case discharges through its own chain and
neither needs the other's `isOk`. -/
lemma switch_8987501505216042171_frame {v : Identifier} {s₀ s₉ : State}
    (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (hv : v ∉ (["_9", "_10", "split_expr_9", "_11", "_12", "split_expr_10", "_13", "_14",
      "split_expr_11", "expr"] : List Identifier))
    (h : A_switch_8987501505216042171 s₀ s₉) : s₉[v]!! = s₀[v]!! := by
  simp only [List.mem_cons, List.not_mem_nil, or_false, not_or] at hv
  obtain ⟨h9, h10, hadd, h11, h12, hld, h13, h14, hld2, hexpr⟩ := hv
  ...
  · rw [hbrA hc] at hnf ⊢
    ...
    have hs5 : isOk s₅ :=
      storage_array_index_access_bytes32_dyn__dyn_isOk h5nf (Spec_ok_unfold hok h5nf h₅)
    have hld2ok : isOk (s₅⟦"split_expr_11" ↦ Clear.EVMState.sload s₅.evm (s₅["_13"]!!)⟧) :=
      isOk_insert.mpr hs5
    have e6 : s₆[v]!! =
        (s₅⟦"split_expr_11" ↦ Clear.EVMState.sload s₅.evm (s₅["_13"]!!)⟧)[v]!! :=
      extract_from_storage_value_dynamict_bytes32_frame hld2ok hnf hexpr
        (Spec_ok_unfold hld2ok hnf h₆)
    have e5 : s₅[v]!! = s₀[v]!! :=
      storage_array_index_access_bytes32_dyn__dyn_frame hok h5nf h13 h14
        (Spec_ok_unfold hok h5nf h₅)
    rw [e6, lookup_insert_of_ne hld2, e5]
  · rw [hbrB hc] at hnf ⊢
    have h3nf : ¬ ❓ s₃ := by
      intro hoo
      exact hnf (Clear.isOutOfFuel_of_Spec_of_isOutOfFuel h₄
    ...
    have hs3 : isOk s₃ :=
      storage_array_index_access_bytes32_dyn__dyn_isOk h3nf (Spec_ok_unfold hs2 h3nf h₃)
    have hldok : isOk (s₃⟦"split_expr_10" ↦ Clear.EVMState.sload s₃.evm (s₃["_11"]!!)⟧) :=
      isOk_insert.mpr hs3
    have e4 : s₄[v]!! =
        (s₃⟦"split_expr_10" ↦ Clear.EVMState.sload s₃.evm (s₃["_11"]!!)⟧)[v]!! :=
      extract_from_storage_value_dynamict_bytes32_frame hldok hnf hexpr
        (Spec_ok_unfold hldok hnf h₄)
    have e3 : s₃[v]!! = s₂[v]!! :=
      storage_array_index_access_bytes32_dyn__dyn_frame hs2 h3nf h11 h12
        (Spec_ok_unfold hs2 h3nf h₃)
    have e2 : s₂[v]!! = s₁[v]!! :=
      checked_add_uint256_frame hs1 h2nf hadd (Spec_ok_unfold hs1 h2nf h₂)
    have e1 : s₁[v]!! = s₀[v]!! :=
      storage_array_index_access_bytes32_dyn__dyn_frame hok h1nf h9 h10
        (Spec_ok_unfold hok h1nf h₁)
    rw [e4, lookup_insert_of_ne hld, e3, e2, e1]


/-- **STORAGE FRAME.**  Both branches only read. -/
lemma switch_8987501505216042171_sload {q : UInt256} {s₀ s₉ : State} (hok : isOk s₀)
    (hnf : ¬ ❓ s₉) (h : A_switch_8987501505216042171 s₀ s₉) :
  ...
  · rw [hbrA hc] at hnf ⊢
    ...
    rw [storage_array_index_access_bytes32_dyn__dyn_sload hok h5nf
      (Spec_ok_unfold hok h5nf h₅)]
  · rw [hbrB hc] at hnf ⊢
    have h3nf : ¬ ❓ s₃ := by
      intro hoo
    ...
    simp only [evm_insert]
    rw [storage_array_index_access_bytes32_dyn__dyn_sload hs2 h3nf
        (Spec_ok_unfold hs2 h3nf h₃),
      checked_add_uint256_sload hs1 h2nf (Spec_ok_unfold hs1 h2nf h₂),
      storage_array_index_access_bytes32_dyn__dyn_sload hok h1nf
        (Spec_ok_unfold hok h1nf h₁)]


/-- **CONFIG FRAME.**  Both branches. -/
lemma switch_8987501505216042171_config {s₀ s₉ : State} (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (hR : RangeInWindow s₀.evm) (hC : CachedInWindow s₀.evm)
    (h : A_switch_8987501505216042171 s₀ s₉) :
  ...
  · rw [hbrA hc] at hnf ⊢
    ...
    have hs5 : isOk s₅ :=
      storage_array_index_access_bytes32_dyn__dyn_isOk h5nf (Spec_ok_unfold hok h5nf h₅)
    obtain ⟨hR5, hC5⟩ := storage_array_index_access_bytes32_dyn__dyn_config hok h5nf hR hC
      (Spec_ok_unfold hok h5nf h₅)
    have hld2ok : isOk (s₅⟦"split_expr_11" ↦ Clear.EVMState.sload s₅.evm (s₅["_13"]!!)⟧) :=
      isOk_insert.mpr hs5
    rw [extract_from_storage_value_dynamict_bytes32_evm hld2ok
  ...
  · rw [hbrB hc] at hnf ⊢
    ...
    have hs3 : isOk s₃ :=
      storage_array_index_access_bytes32_dyn__dyn_isOk h3nf (Spec_ok_unfold hs2 h3nf h₃)
    obtain ⟨hR1, hC1⟩ := storage_array_index_access_bytes32_dyn__dyn_config hok h1nf hR hC
      (Spec_ok_unfold hok h1nf h₁)
    obtain ⟨hR2, hC2⟩ := checked_add_uint256_config hs1 h2nf hR1 hC1
      (Spec_ok_unfold hs1 h2nf h₂)
    obtain ⟨hR3, hC3⟩ := storage_array_index_access_bytes32_dyn__dyn_config hs2 h3nf hR2 hC2
      (Spec_ok_unfold hs2 h3nf h₃)
    have hldok : isOk (s₃⟦"split_expr_10" ↦ Clear.EVMState.sload s₃.evm (s₃["_11"]!!)⟧) :=
      isOk_insert.mpr hs3
    rw [extract_from_storage_value_dynamict_bytes32_evm hldok (Spec_ok_unfold hldok hnf h₄)]
