...
def ACond_for_6561856544793224737 (s₀ : State) : Literal := fromBool (s₀["i"]!! < 1)
def APost_for_6561856544793224737 (s₀ s₉ : State) : Prop := True
def ABody_for_6561856544793224737 (s₀ s₉ : State) : Prop := True
def AFor_for_6561856544793224737 (s₀ s₉ : State) : Prop := True
...
lemma for_6561856544793224737_concrete_of_post_abs {s₀ s₉ : State} :
  ...
  unfold APost_for_6561856544793224737
...
lemma for_6561856544793224737_concrete_of_body_abs {s₀ s₉ : State} :
  ...
  unfold ABody_for_6561856544793224737
  rcases s₀ with ⟨evm, varstore⟩ | _ | _ <;> aesop_spec

lemma AZero_for_6561856544793224737 : ∀ s₀, isOk s₀ → ACond_for_6561856544793224737 (👌 s₀) = 0 → AFor_for_6561856544793224737 s₀ s₀ := by
  intro s₀ _ _
  trivial
lemma AOk_for_6561856544793224737 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isOk s₂ → ¬ ❓ s₅ → ¬ ACond_for_6561856544793224737 s₀ = 0 → ABody_for_6561856544793224737 s₀ s₂ → APost_for_6561856544793224737 s₂ s₄ → Spec AFor_for_6561856544793224737 s₄ s₅ → AFor_for_6561856544793224737 s₀ s₅
:= by
