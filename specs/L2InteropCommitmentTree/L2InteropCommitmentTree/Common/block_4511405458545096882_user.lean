...
import generated.L2InteropCommitmentTree.L2InteropCommitmentTree.Common.block_4511405458545096882_gen
...
def A_block_4511405458545096882 (s₀ s₉ : State) : Prop :=
  let a := s₀⟦"_1" ↦ s₀["var_self_slot"]!! + 2⟧
  ∃ s₁, Spec (A_storage_array_index_access_bytes32_dyn__dyn_5278 "_2" "_3" (a["_1"]!!)) a s₁ ∧
    ∃ s₂, Spec (A_storage_array_index_access_bytes32_dyn__dyn "_4" "_5" (s₁["_2"]!!) (s₁["var_index"]!!)) s₁ s₂ ∧
      ∃ s₃, Spec (A_update_storage_value_bytes32_to_bytes32
          (s₂["_4"]!!) (s₂["_5"]!!) (s₂["var_itemHash"]!!)) s₂ s₃ ∧
        s₉ = s₃⟦"var_currentHash" ↦ s₃["var_itemHash"]!!⟧

lemma block_4511405458545096882_abs_of_concrete {s₀ s₉ : State} :
  Spec block_4511405458545096882_concrete_of_code s₀ s₉ →
  Spec A_block_4511405458545096882 s₀ s₉ := by
  unfold block_4511405458545096882_concrete_of_code A_block_4511405458545096882
...
lemma block_4511405458545096882_isOk {s₀ s₉ : State} (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (h : A_block_4511405458545096882 s₀ s₉) : isOk s₉ := by
  ...
  have hs1 : isOk s₁ :=
    storage_array_index_access_bytes32_dyn__dyn_5278_isOk h1nf (Spec_ok_unfold hain h1nf h₁)
  have hs2 : isOk s₂ :=
...
lemma block_4511405458545096882_not_break {s₀ s₉ : State} (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (h : A_block_4511405458545096882 s₀ s₉) : ¬ isBreak s₉ :=
  fun hb => not_isOk_of_isBreak hb (block_4511405458545096882_isOk hok hnf h)
