...
lemma switch_3698598331043311763_frame {v : Identifier} {s₀ s₉ : State}
    (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (hv : v ∉ (["_10", "_11", "split_expr_10", "_12", "_13", "split_expr_11",
      "split_expr_12", "_14", "_15", "split_expr_13", "expr"] : List Identifier))
    (h : A_switch_3698598331043311763 s₀ s₉) : s₉[v]!! = s₀[v]!! := by
  ...
  · rw [hbrA hc] at hnf ⊢
    ...
    have hs5 : isOk s₅ :=
      storage_array_index_access_bytes32_dyn__dyn_isOk h5nf (Spec_ok_unfold hddok h5nf h₅)
    ...
    have e5 : s₅[v]!! = (s₀⟦"split_expr_12" ↦ s₀["var_self_slot"]!! + 3⟧)[v]!! :=
      storage_array_index_access_bytes32_dyn__dyn_frame hddok h5nf h14 h15
  ...
  · rw [hbrB hc] at hnf ⊢
    ...
    have e3 : s₃[v]!! = s₂[v]!! :=
      storage_array_index_access_bytes32_dyn__dyn_frame hs2 h3nf h12 h13
        (Spec_ok_unfold hs2 h3nf h₃)
    have e2 : s₂[v]!! = s₁[v]!! :=
      checked_add_uint256_frame hs1 h2nf hadd (Spec_ok_unfold hs1 h2nf h₂)
    have e1 : s₁[v]!! = s₀[v]!! :=
      storage_array_index_access_bytes32_dyn__dyn_frame hok h1nf h10 h11
...
lemma switch_3698598331043311763_sload {q : UInt256} {s₀ s₉ : State} (hok : isOk s₀)
    (hnf : ¬ ❓ s₉) (h : A_switch_3698598331043311763 s₀ s₉) :
  ...
  · rw [hbrA hc] at hnf ⊢
    ...
    rw [storage_array_index_access_bytes32_dyn__dyn_sload hddok h5nf
      (Spec_ok_unfold hddok h5nf h₅)]
    simp only [evm_insert]
  · rw [hbrB hc] at hnf ⊢
    have h3nf : ¬ ❓ s₃ := by
...
lemma switch_3698598331043311763_config {s₀ s₉ : State} (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (hR : RangeInWindow s₀.evm) (hC : CachedInWindow s₀.evm)
    (h : A_switch_3698598331043311763 s₀ s₉) :
  ...
  · rw [hbrA hc] at hnf ⊢
    ...
    have hs5 : isOk s₅ :=
      storage_array_index_access_bytes32_dyn__dyn_isOk h5nf (Spec_ok_unfold hddok h5nf h₅)
    obtain ⟨hR5, hC5⟩ := storage_array_index_access_bytes32_dyn__dyn_config hddok h5nf
  ...
  · rw [hbrB hc] at hnf ⊢
    ...
    have h2nf : ¬ ❓ s₂ := fun hoo => h3nf (Clear.isOutOfFuel_of_Spec_of_isOutOfFuel h₃ hoo)
    have h1nf : ¬ ❓ s₁ := fun hoo => h2nf (Clear.isOutOfFuel_of_Spec_of_isOutOfFuel h₂ hoo)
    have hs1 : isOk s₁ := storage_array_index_access_bytes32_dyn__dyn_isOk h1nf (Spec_ok_unfold hok h1nf h₁)
    have hs2 : isOk s₂ := checked_add_uint256_isOk h2nf (Spec_ok_unfold hs1 h2nf h₂)
    have hs3 : isOk s₃ := storage_array_index_access_bytes32_dyn__dyn_isOk h3nf (Spec_ok_unfold hs2 h3nf h₃)
    obtain ⟨hR1, hC1⟩ := storage_array_index_access_bytes32_dyn__dyn_config hok h1nf hR hC (Spec_ok_unfold hok h1nf h₁)
    obtain ⟨hR2, hC2⟩ := checked_add_uint256_config hs1 h2nf hR1 hC1
      (Spec_ok_unfold hs1 h2nf h₂)
    obtain ⟨hR3, hC3⟩ := storage_array_index_access_bytes32_dyn__dyn_config hs2 h3nf hR2 hC2 (Spec_ok_unfold hs2 h3nf h₃)
    have hldok : isOk (s₃⟦"split_expr_11" ↦ Clear.EVMState.sload s₃.evm (s₃["_12"]!!)⟧) :=
      isOk_insert.mpr hs3
    rw [extract_from_storage_value_dynamict_bytes32_evm hldok (Spec_ok_unfold hldok hnf h₄)]
    simp only [evm_insert]
