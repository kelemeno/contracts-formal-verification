...
import generated.L2InteropCommitmentTree.L2InteropCommitmentTree.Common.block_2936313771097161809_gen
...
def A_block_2936313771097161809 (s₀ s₉ : State) : Prop :=
  ∃ s₁, Spec (A_checked_div_uint256 "var_index" (s₀["var_index"]!!)) s₀ s₁ ∧
    ∃ s₂, Spec (A_checked_div_uint256 "var_maxNodeNumber" (s₁["var_maxNodeNumber"]!!)) s₁ s₂ ∧
      ∃ s₃, Spec (A_checked_add_uint256 "split_expr_14" (s₂["var_i"]!!)) s₂ s₃ ∧
        ∃ s₄, Spec (A_storage_array_index_access_bytes32_dyn__dyn "_16" "_17" (s₃["_1"]!!)
            (s₃["split_expr_14"]!!)) s₃ s₄ ∧
          ∃ s₅, Spec (A_storage_array_index_access_bytes32_dyn__dyn "_18" "_19"
              (s₄["_16"]!!) (s₄["var_index"]!!)) s₄ s₅ ∧
            s₉ = s₅

lemma block_2936313771097161809_abs_of_concrete {s₀ s₉ : State} :
  Spec block_2936313771097161809_concrete_of_code s₀ s₉ →
  Spec A_block_2936313771097161809 s₀ s₉ := by
  unfold block_2936313771097161809_concrete_of_code A_block_2936313771097161809
...
lemma block_2936313771097161809_isOk {s₀ s₉ : State} (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (h : A_block_2936313771097161809 s₀ s₉) : isOk s₉ := by
...
lemma block_2936313771097161809_not_break {s₀ s₉ : State} (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (h : A_block_2936313771097161809 s₀ s₉) : ¬ isBreak s₉ :=
  fun hb => not_isOk_of_isBreak hb (block_2936313771097161809_isOk hok hnf h)
...
lemma block_2936313771097161809_index {s₀ s₉ : State} (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (h : A_block_2936313771097161809 s₀ s₉) :
  ...
  have e4 : s₄["var_index"]!! = s₃["var_index"]!! :=
    storage_array_index_access_bytes32_dyn__dyn_frame hs3 h4nf (by decide) (by decide)
      (Spec_ok_unfold hs3 h4nf h₄)
  have e5 : s₅["var_index"]!! = s₄["var_index"]!! :=
    storage_array_index_access_bytes32_dyn__dyn_frame hs4 hnf (by decide) (by decide)
...
lemma block_2936313771097161809_frame {v : Identifier} {s₀ s₉ : State}
    (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (hv : v ∉ (["var_index", "var_maxNodeNumber", "split_expr_14", "_16", "_17", "_18",
      "_19"] : List Identifier))
    (h : A_block_2936313771097161809 s₀ s₉) : s₉[v]!! = s₀[v]!! := by
  ...
  have e4 : s₄[v]!! = s₃[v]!! :=
    storage_array_index_access_bytes32_dyn__dyn_frame hs3 h4nf h15 h16
      (Spec_ok_unfold hs3 h4nf h₄)
  have e5 : s₅[v]!! = s₄[v]!! :=
    storage_array_index_access_bytes32_dyn__dyn_frame hs4 hnf h17 h18
      (Spec_ok_unfold hs4 hnf h₅)
  rw [e5, e4, e3, e2, e1]
...
lemma block_2936313771097161809_sload {q : UInt256} {s₀ s₉ : State} (hok : isOk s₀)
    (hnf : ¬ ❓ s₉) (h : A_block_2936313771097161809 s₀ s₉) :
  ...
  have hs4 : isOk s₄ :=
    storage_array_index_access_bytes32_dyn__dyn_isOk h4nf (Spec_ok_unfold hs3 h4nf h₄)
  rw [storage_array_index_access_bytes32_dyn__dyn_sload hs4 hnf (Spec_ok_unfold hs4 hnf h₅),
    storage_array_index_access_bytes32_dyn__dyn_sload hs3 h4nf (Spec_ok_unfold hs3 h4nf h₄),
...
lemma block_2936313771097161809_config {s₀ s₉ : State} (hok : isOk s₀) (hnf : ¬ ❓ s₉)
  ...
  obtain ⟨hR4, hC4⟩ := storage_array_index_access_bytes32_dyn__dyn_config hs3 h4nf hR3 hC3
    (Spec_ok_unfold hs3 h4nf h₄)
  exact storage_array_index_access_bytes32_dyn__dyn_config hs4 hnf hR4 hC4
    (Spec_ok_unfold hs4 hnf h₅)
