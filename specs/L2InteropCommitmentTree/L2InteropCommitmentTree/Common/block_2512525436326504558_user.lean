...
```
    expr_mpos := mload(64)      -- the free pointer, p
...
def A_block_2512525436326504558 (s₀ s₉ : State) : Prop :=
  let a := s₀⟦"expr_mpos" ↦ Clear.EVMState.mload s₀.evm 64⟧
  let b := a⟦"_4" ↦ a["expr_mpos"]!! + 32⟧
  let m1 := b🇪⟦Clear.EVMState.mstore s₀.evm (b["_4"]!!) (b["_1"]!!)⟧
  let c := m1⟦"split_expr_2" ↦ m1["expr_mpos"]!! + 64⟧
...
lemma block_2512525436326504558_abs_of_concrete {s₀ s₉ : State} :
  ...
  unfold block_2512525436326504558_concrete_of_code A_block_2512525436326504558
