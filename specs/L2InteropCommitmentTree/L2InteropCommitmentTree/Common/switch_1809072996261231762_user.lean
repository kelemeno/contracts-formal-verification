...
- even (default): the node is a LEFT child.  Select the sibling via
  `switch_3698598331043311763` — the neighbour at `index + 1`, or the level's empty default — and hash `(currentHash, sibling)`.
...
same leaves. -/
def A_switch_1809072996261231762 (s₀ s₉ : State) : Prop :=
  ∃ s₁, Spec A_block_3445626554890327039 s₀ s₁ ∧
    ∃ s₂, Spec A_block_896716371604423710 s₁ s₂ ∧
      ∃ s₃, Spec A_switch_3698598331043311763 (s₀⟦"expr" ↦ 0⟧) s₃ ∧
...
lemma switch_1809072996261231762_abs_of_concrete {s₀ s₉ : State} :
  ...
  unfold switch_1809072996261231762_concrete_of_code A_switch_1809072996261231762
...
lemma switch_1809072996261231762_isOk {s₀ s₉ : State} (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (h : A_switch_1809072996261231762 s₀ s₉) : isOk s₉ := by
  ...
  · rw [heven he] at hnf ⊢
    ...
    have hs3 : isOk s₃ :=
      switch_3698598331043311763_isOk hexpr h3nf (Spec_ok_unfold hexpr h3nf h₃)
  ...
  · rw [hodd he] at hnf ⊢
    ...
    have hs1 : isOk s₁ := block_3445626554890327039_isOk hok h1nf (Spec_ok_unfold hok h1nf h₁)
    exact block_896716371604423710_isOk hs1 hnf (Spec_ok_unfold hs1 hnf h₂)

lemma switch_1809072996261231762_not_break {s₀ s₉ : State} (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (h : A_switch_1809072996261231762 s₀ s₉) : ¬ isBreak s₉ :=
  fun hb => not_isOk_of_isBreak hb (switch_1809072996261231762_isOk hok hnf h)
...
lemma switch_1809072996261231762_frame {v : Identifier} {s₀ s₉ : State}
    (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    ...
    (h : A_switch_1809072996261231762 s₀ s₉) : s₉[v]!! = s₀[v]!! := by
  ...
  · rw [hbrEven hc] at hnf ⊢
    ...
    have h3nf : ¬ ❓ s₃ := fun hoo => hnf (Clear.isOutOfFuel_of_Spec_of_isOutOfFuel h₄ hoo)
    have hs3 : isOk s₃ :=
      switch_3698598331043311763_isOk hins h3nf (Spec_ok_unfold hins h3nf h₃)
    have e4 : s₄[v]!! = s₃[v]!! :=
      fun_efficientHash_frame hs3 hnf hcur (Spec_ok_unfold hs3 hnf h₄)
    have e3 : s₃[v]!! = (s₀⟦"expr" ↦ 0⟧)[v]!! :=
      switch_3698598331043311763_frame hins h3nf hL2003 (Spec_ok_unfold hins h3nf h₃)
    rw [e4, e3, lookup_insert_of_ne hexpr]
  · rw [hbrOdd hc] at hnf ⊢
    have h1nf : ¬ ❓ s₁ := fun hoo => hnf (Clear.isOutOfFuel_of_Spec_of_isOutOfFuel h₂ hoo)
    have hs1 : isOk s₁ :=
      block_3445626554890327039_isOk hok h1nf (Spec_ok_unfold hok h1nf h₁)
    have e2 : s₂[v]!! = s₁[v]!! :=
      block_896716371604423710_frame hs1 hnf hcur (Spec_ok_unfold hs1 hnf h₂)
    have e1 : s₁[v]!! = s₀[v]!! :=
      block_3445626554890327039_frame hok h1nf hL5648 (Spec_ok_unfold hok h1nf h₁)
    rw [e2, e1]


/-- **STORAGE FRAME.**  The parity switch reads siblings and hashes; no slot changes. -/
lemma switch_1809072996261231762_sload {q : UInt256} {s₀ s₉ : State} (hok : isOk s₀)
    (hnf : ¬ ❓ s₉) (h : A_switch_1809072996261231762 s₀ s₉) :
  ...
  · rw [hbrEven hc] at hnf ⊢
    ...
    rw [fun_efficientHash_sload hs3 (Spec_ok_unfold hs3 hnf h₄),
      switch_3698598331043311763_sload hins h3nf (Spec_ok_unfold hins h3nf h₃)]
  ...
  · rw [hbrOdd hc] at hnf ⊢
    ...
    rw [block_896716371604423710_sload hs1 hnf (Spec_ok_unfold hs1 hnf h₂),
      block_3445626554890327039_sload hok h1nf (Spec_ok_unfold hok h1nf h₁)]


/-- **CONFIG FRAME.**  The parity switch, both branches. -/
lemma switch_1809072996261231762_config {s₀ s₉ : State} (hok : isOk s₀) (hnf : ¬ ❓ s₉)
    (hR : RangeInWindow s₀.evm) (hC : CachedInWindow s₀.evm)
    (h : A_switch_1809072996261231762 s₀ s₉) :
  ...
  · rw [hbrEven hc] at hnf ⊢
    ...
    have hs3 : isOk s₃ :=
      switch_3698598331043311763_isOk hins h3nf (Spec_ok_unfold hins h3nf h₃)
    obtain ⟨hR3, hC3⟩ := switch_3698598331043311763_config hins h3nf
  ...
  · rw [hbrOdd hc] at hnf ⊢
    ...
    have hs1 : isOk s₁ :=
      block_3445626554890327039_isOk hok h1nf (Spec_ok_unfold hok h1nf h₁)
    obtain ⟨hR1, hC1⟩ := block_3445626554890327039_config hok h1nf hR hC
