...
def A_finalize_allocation_5176  (memPtr : Literal) (s₀ s₉ : State) : Prop := finalize_allocation_5176_concrete_of_code.1  memPtr s₀ s₉
...
lemma finalize_allocation_5176_abs_of_concrete {s₀ s₉ : State} { memPtr} :
  ...
  intro h
  simpa [A_finalize_allocation_5176] using h
