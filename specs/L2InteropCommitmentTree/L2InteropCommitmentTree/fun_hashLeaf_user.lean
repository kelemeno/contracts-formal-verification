...
def A_fun_hashLeaf (var : Identifier) (var_leaf_mpos : Literal) (s₀ s₉ : State) : Prop :=
  ∃ s₁, Spec L2InteropCommitmentTree.Common.A_block_1948431615937796266
      (s₀☎️⟦["var_leaf_mpos"],[var_leaf_mpos]⟧) s₁ ∧
    ∃ s₂, Spec L2InteropCommitmentTree.Common.A_block_2512525436326504558 s₁ s₂ ∧
      ∃ s₃, Spec L2InteropCommitmentTree.Common.A_block_5488197900316908801 s₂ s₃ ∧
        ∃ s₄, Spec L2InteropCommitmentTree.Common.A_block_7615139809432579602 s₃ s₄ ∧
          s₉ = 🧟s₄🏪⟦s₀⟧⟦var ↦ s₄["var"]!!⟧
