...
Part H. -/
def A_fun_updateLeaf_5205 (var : Identifier) (var_index var_itemHash : Literal)
    (s₀ s₉ : State) : Prop :=
  ...
  ∃ s₁, Spec (A_checked_sub_uint256 "var_maxNodeNumber" (g["split_expr_0"]!!)) g s₁ ∧
    ∃ s₂, Spec L2InteropCommitmentTree.Common.A_if_4451958921457272093 s₁ s₂ ∧
      ∃ s₃, Spec L2InteropCommitmentTree.Common.A_block_8102603259148870207 s₂ s₃ ∧
        ∃ s₄, Spec L2InteropCommitmentTree.Common.A_block_5752024616743232143 s₃ s₄ ∧
          ∃ s₅, Spec L2InteropCommitmentTree.Common.AFor_for_2268004712116198193 s₄ s₅ ∧
