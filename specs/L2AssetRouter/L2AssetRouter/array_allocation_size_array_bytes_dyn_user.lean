...
def A_array_allocation_size_array_bytes_dyn (size : Identifier) (length : Literal) (s₀ s₉ : State) : Prop := sorry
...
lemma array_allocation_size_array_bytes_dyn_abs_of_concrete {s₀ s₉ : State} {size length} :
  ...
  unfold array_allocation_size_array_bytes_dyn_concrete_of_code A_array_allocation_size_array_bytes_dyn
  sorry
