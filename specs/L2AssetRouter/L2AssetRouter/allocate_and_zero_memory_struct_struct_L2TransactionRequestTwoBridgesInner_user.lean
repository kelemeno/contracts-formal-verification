import Clear.ReasoningPrinciple

import generated.L2AssetRouter.L2AssetRouter.Common.block_2377343992131253793
import generated.L2AssetRouter.L2AssetRouter.finalize_allocation_11803
import generated.L2AssetRouter.L2AssetRouter.Common.block_4257078710847412873
import generated.L2AssetRouter.L2AssetRouter.Common.block_3031367666615557326

import generated.L2AssetRouter.L2AssetRouter.allocate_and_zero_memory_struct_struct_L2TransactionRequestTwoBridgesInner_gen


namespace generated.L2AssetRouter.L2AssetRouter

section

open Clear EVMState Ast Expr Stmt FunctionDefinition State Interpreter ExecLemmas OutOfFuelLemmas Abstraction YulNotation PrimOps ReasoningPrinciple Utilities L2AssetRouter.Common generated.L2AssetRouter L2AssetRouter

def A_allocate_and_zero_memory_struct_struct_L2TransactionRequestTwoBridgesInner (memPtr : Identifier)  (s₀ s₉ : State) : Prop := allocate_and_zero_memory_struct_struct_L2TransactionRequestTwoBridgesInner_concrete_of_code.1 memPtr s₀ s₉

lemma allocate_and_zero_memory_struct_struct_L2TransactionRequestTwoBridgesInner_abs_of_concrete {s₀ s₉ : State} {memPtr } :
  Spec (allocate_and_zero_memory_struct_struct_L2TransactionRequestTwoBridgesInner_concrete_of_code.1 memPtr ) s₀ s₉ →
  Spec (A_allocate_and_zero_memory_struct_struct_L2TransactionRequestTwoBridgesInner memPtr ) s₀ s₉ := by
  intro h
  simpa [A_allocate_and_zero_memory_struct_struct_L2TransactionRequestTwoBridgesInner] using h

end

end generated.L2AssetRouter.L2AssetRouter
