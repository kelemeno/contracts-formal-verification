...
def A_calldata_array_index_range_access_bytes_calldata (offsetOut lengthOut : Identifier) (offset length startIndex endIndex : Literal) (s₀ s₉ : State) : Prop := calldata_array_index_range_access_bytes_calldata_concrete_of_code.1 offsetOut lengthOut offset length startIndex endIndex s₀ s₉
...
lemma calldata_array_index_range_access_bytes_calldata_abs_of_concrete {s₀ s₉ : State} {offsetOut lengthOut offset length startIndex endIndex} :
  ...
  intro h
  simpa [A_calldata_array_index_range_access_bytes_calldata] using h
