...
def A_fun_recoverAtomicCall_inner (var_recovered : Identifier) (var_destChainId var_callData_offset var_callData_length : Literal) (s₀ s₉ : State) : Prop := fun_recoverAtomicCall_inner_concrete_of_code.1 var_recovered var_destChainId var_callData_offset var_callData_length s₀ s₉
...
lemma fun_recoverAtomicCall_inner_abs_of_concrete {s₀ s₉ : State} {var_recovered var_destChainId var_callData_offset var_callData_length} :
  ...
  intro h
  simpa [A_fun_recoverAtomicCall_inner] using h
