...
def A_calldata_array_index_access_bytes_calldata_11804 (addr : Identifier) (base_ref length : Literal) (s₀ s₉ : State) : Prop := calldata_array_index_access_bytes_calldata_11804_concrete_of_code.1 addr base_ref length s₀ s₉
...
lemma calldata_array_index_access_bytes_calldata_11804_abs_of_concrete {s₀ s₉ : State} {addr base_ref length} :
  ...
  intro h
  simpa [A_calldata_array_index_access_bytes_calldata_11804] using h
