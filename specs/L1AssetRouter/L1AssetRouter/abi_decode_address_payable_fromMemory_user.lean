...
def A_abi_decode_address_payable_fromMemory
    (value : Identifier) (offset : Literal) (s₀ s₉ : State) : Prop :=
  abi_decode_address_payable_fromMemory_concrete_of_code.1 value offset s₀ s₉
...
lemma abi_decode_address_payable_fromMemory_abs_of_concrete {s₀ s₉ : State} {value offset} :
  ...
  intro h
  simpa [A_abi_decode_address_payable_fromMemory] using h
