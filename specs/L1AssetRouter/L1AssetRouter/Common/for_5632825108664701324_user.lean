...
def ACond_for_5632825108664701324 (s₀ : State) : Literal :=
  fromBool (s₀["i"]!! < s₀["_1"]!!)

/-- Loop post: `i := add(i, 32)`. -/
def APost_for_5632825108664701324 (s₀ s₉ : State) : Prop :=
...
and post. -/
def AFor_for_5632825108664701324 (s₀ s₉ : State) : Prop :=
  ∀ evm store, s₉ = Ok evm store → ¬ ((Ok evm store)["i"]!! < (Ok evm store)["_1"]!!)

/-- Loop body: write the constant `96` one word past `memPtr_1 + {i}`.

    let split_expr_5 := add(memPtr_1, i);  let split_expr_6 := add(split_expr_5, 32);  mstore(split_expr_6, 96) -/
def ABody_for_5632825108664701324 (s₀ s₉ : State) : Prop :=
  ∀ evm store, s₀ = Ok evm store →
    let s₁ := (Ok evm store)⟦"split_expr_5" ↦ ((Ok evm store)["memPtr_1"]!! + ((Ok evm store)["i"]!!))⟧
    let s₂ := s₁⟦"split_expr_6" ↦ (s₁["split_expr_5"]!! + 32)⟧
    s₉ = s₂🇪⟦EVMState.mstore (Ok evm store).evm (s₂["split_expr_6"]!!) 96⟧
...
lemma for_5632825108664701324_concrete_of_post_abs {s₀ s₉ : State} :
  ...
  unfold for_5632825108664701324_post_concrete_of_code APost_for_5632825108664701324
...
lemma AZero_for_5632825108664701324 : ∀ s₀, isOk s₀ → ACond_for_5632825108664701324 (👌 s₀) = 0 → AFor_for_5632825108664701324 s₀ s₀ := by
  intro s₀ hok hcond
  unfold AFor_for_5632825108664701324 ACond_for_5632825108664701324 at *
  intro evm store hs
  subst hs
  intro hlt
...
lemma AOk_for_5632825108664701324 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isOk s₂ → ¬ ❓ s₅ → ¬ ACond_for_5632825108664701324 s₀ = 0 → ABody_for_5632825108664701324 s₀ s₂ → APost_for_5632825108664701324 s₂ s₄ → Spec AFor_for_5632825108664701324 s₄ s₅ → AFor_for_5632825108664701324 s₀ s₅ := by
  ...
  · have h4 : s₄ = (Ok e2 st2)⟦"i" ↦ ((Ok e2 st2)["i"]!! + 32)⟧ := hpost e2 st2 rfl
    ...
    exact Spec_ok_unfold (P := AFor_for_5632825108664701324) (s := s₄) (s' := s₅) hok4 h5 hspec
...
lemma AContinue_for_5632825108664701324 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isContinue s₂ → ¬ ACond_for_5632825108664701324 s₀ = 0 → ABody_for_5632825108664701324 s₀ s₂ → Spec APost_for_5632825108664701324 (🧟s₂) s₄ → Spec AFor_for_5632825108664701324 s₄ s₅ → AFor_for_5632825108664701324 s₀ s₅ := by
  ...
  · exact Spec_ok_unfold (P := AFor_for_5632825108664701324) (s := Ok e4 st4) (s' := s₅) (by simp [isOk]) h5 hspec evm store hs
...
lemma for_5632825108664701324_concrete_of_body_abs {s₀ s₉ : State} :
  ...
  unfold for_5632825108664701324_body_concrete_of_code ABody_for_5632825108664701324
  rcases s₀ with ⟨evm, store⟩ | _ | _ <;> [skip; aesop_spec; aesop_spec]
  apply spec_eq
  intro _hne hc
  intro evmA storeA hok
