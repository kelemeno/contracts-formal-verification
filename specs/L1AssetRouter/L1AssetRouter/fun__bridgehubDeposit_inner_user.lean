...
def A_fun__bridgehubDeposit_inner (var_request_mpos : Identifier) (var_chainId var_originalCaller var_value var_data_offset var_data_length var_nativeTokenVault : Literal) (s₀ s₉ : State) : Prop :=
  fun__bridgehubDeposit_inner_concrete_of_code.1 var_request_mpos var_chainId var_originalCaller var_value var_data_offset var_data_length var_nativeTokenVault s₀ s₉
...
lemma fun__bridgehubDeposit_inner_abs_of_concrete {s₀ s₉ : State} {var_request_mpos var_chainId var_originalCaller var_value var_data_offset var_data_length var_nativeTokenVault} :
  ...
  intro h
  simpa [A_fun__bridgehubDeposit_inner] using h
