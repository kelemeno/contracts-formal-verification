...
def A_fun_ensureTokenRegisteredWithNTV (var_assetId : Identifier) (var_token : Literal) (s₀ s₉ : State) : Prop :=
  fun_ensureTokenRegisteredWithNTV_concrete_of_code.1 var_assetId var_token s₀ s₉
...
lemma fun_ensureTokenRegisteredWithNTV_abs_of_concrete {s₀ s₉ : State} {var_assetId var_token} :
  ...
  intro h
  simpa [A_fun_ensureTokenRegisteredWithNTV] using h
