...
import generated.InteropHandler.InteropHandler.Common.block_1556246446440514414_gen
...
Self-contained: does not mention `block_1556246446440514414_concrete_of_code`.
-/
def A_block_1556246446440514414 (s₀ s₉ : State) : Prop :=
  s₉ = (((((s₀⟦"split_expr_0" ↦ (s₀.evm.sload (s₀["dataSlot"]!!))⟧)⟦"split_expr_1" ↦ UInt256.lnot (255 : UInt256)⟧)⟦"split_expr_2" ↦ Fin.land (s₀.evm.sload (s₀["dataSlot"]!!)) (UInt256.lnot (255 : UInt256))⟧)⟦"split_expr_3" ↦ Fin.lor (Fin.land (s₀.evm.sload (s₀["dataSlot"]!!)) (UInt256.lnot (255 : UInt256))) (2 : UInt256)⟧)🇪⟦(s₀.evm.sstore (s₀["dataSlot"]!!) (Fin.lor (Fin.land (s₀.evm.sload (s₀["dataSlot"]!!)) (UInt256.lnot (255 : UInt256))) (2 : UInt256)))⟧)

lemma block_1556246446440514414_abs_of_concrete {s₀ s₉ : State} :
  Spec block_1556246446440514414_concrete_of_code s₀ s₉ →
  Spec A_block_1556246446440514414 s₀ s₉ := by
  unfold block_1556246446440514414_concrete_of_code A_block_1556246446440514414
