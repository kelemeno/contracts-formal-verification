...
Abstract spec for the Yul block
    ...
    {
      let split_expr_12 := add(offset_1, 68)
      let split_expr_13 := calldataload(split_expr_12)
      mstore(split_expr_11, split_expr_13)
      let split_expr_14 := add(offset_1, 100)
      let offset_2 := calldataload(split_expr_14)
    }
...
Self-contained: does not mention `block_8649141812215965287_concrete_of_code`.
-/
def A_block_8649141812215965287 (s₀ s₉ : State) : Prop :=
  s₉ = (((((s₀⟦"split_expr_12" ↦ (s₀["offset_1"]!!) + (68 : UInt256)⟧)⟦"split_expr_13" ↦ (s₀.evm.calldataload ((s₀["offset_1"]!!) + (68 : UInt256)))⟧)⟦"split_expr_14" ↦ (s₀["offset_1"]!!) + (100 : UInt256)⟧)⟦"offset_2" ↦ ((s₀.evm.mstore (s₀["split_expr_11"]!!) (s₀.evm.calldataload ((s₀["offset_1"]!!) + (68 : UInt256)))).calldataload ((s₀["offset_1"]!!) + (100 : UInt256)))⟧)🇪⟦(s₀.evm.mstore (s₀["split_expr_11"]!!) (s₀.evm.calldataload ((s₀["offset_1"]!!) + (68 : UInt256))))⟧)
...
lemma block_8649141812215965287_abs_of_concrete {s₀ s₉ : State} :
  ...
  unfold block_8649141812215965287_concrete_of_code A_block_8649141812215965287
