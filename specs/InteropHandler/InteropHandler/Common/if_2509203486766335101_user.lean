...
import generated.InteropHandler.InteropHandler.Common.if_2509203486766335101_gen
...
`v = 0`, so the body runs precisely when `split_expr_2` is zero.  On that branch every
...
Self-contained: does not mention `if_2509203486766335101_concrete_of_code`.
-/
def A_if_2509203486766335101 (s₀ s₉ : State) : Prop :=
  ((s₀["split_expr_2"]!!) = 0 → s₉ = ((s₀⟦"var_value" ↦ (s₀["result_2"]!!)⟧)⟦"var_result" ↦ (s₀["var_result"]!!) + (4 : UInt256)⟧))
  ∧ (¬ ((s₀["split_expr_2"]!!) = 0) → s₉ = s₀)

lemma if_2509203486766335101_abs_of_concrete {s₀ s₉ : State} :
  Spec if_2509203486766335101_concrete_of_code s₀ s₉ →
  Spec A_if_2509203486766335101 s₀ s₉ := by
  unfold if_2509203486766335101_concrete_of_code A_if_2509203486766335101
