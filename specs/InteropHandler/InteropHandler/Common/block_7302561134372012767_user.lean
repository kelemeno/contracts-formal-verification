...
import generated.InteropHandler.InteropHandler.Common.block_7302561134372012767_gen
...
Abstract spec for the Yul block
    ...
    {
      let split_expr_17 := add(var_proof_mpos, 32)
      let _7 := mload(split_expr_17)
      let split_expr_18 := add(var_proof_mpos, 64)
      let _8 := mload(split_expr_18)
      let _mpos := mload(_2)
    }
...
Self-contained: does not mention `block_7302561134372012767_concrete_of_code`.
-/
def A_block_7302561134372012767 (s₀ s₉ : State) : Prop :=
  s₉ = (((((s₀⟦"split_expr_17" ↦ (s₀["var_proof_mpos"]!!) + (32 : UInt256)⟧)⟦"_7" ↦ (s₀.evm.mload ((s₀["var_proof_mpos"]!!) + (32 : UInt256)))⟧)⟦"split_expr_18" ↦ (s₀["var_proof_mpos"]!!) + (64 : UInt256)⟧)⟦"_8" ↦ (s₀.evm.mload ((s₀["var_proof_mpos"]!!) + (64 : UInt256)))⟧)⟦"_mpos" ↦ (s₀.evm.mload (s₀["_2"]!!))⟧)

lemma block_7302561134372012767_abs_of_concrete {s₀ s₉ : State} :
  Spec block_7302561134372012767_concrete_of_code s₀ s₉ →
  Spec A_block_7302561134372012767 s₀ s₉ := by
  unfold block_7302561134372012767_concrete_of_code A_block_7302561134372012767
