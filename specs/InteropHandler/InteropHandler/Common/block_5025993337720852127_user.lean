...
import generated.InteropHandler.InteropHandler.Common.block_5025993337720852127_gen
...
Abstract spec for the Yul block
    ...
    {
      ret := sum
      let split_expr_22 := shl(160, 1)
      let split_expr_23 := sub(split_expr_22, 1)
      let _5 := and(sum, split_expr_23)
      let _6 := mload(_3)
    }
...
Self-contained: does not mention `block_5025993337720852127_concrete_of_code`.
-/
def A_block_5025993337720852127 (s₀ s₉ : State) : Prop :=
  s₉ = (((((s₀⟦"ret" ↦ (s₀["sum"]!!)⟧)⟦"split_expr_22" ↦ Fin.shiftLeft (1 : UInt256) (160 : UInt256)⟧)⟦"split_expr_23" ↦ (Fin.shiftLeft (1 : UInt256) (160 : UInt256)) - (1 : UInt256)⟧)⟦"_5" ↦ Fin.land (s₀["sum"]!!) ((Fin.shiftLeft (1 : UInt256) (160 : UInt256)) - (1 : UInt256))⟧)⟦"_6" ↦ (s₀.evm.mload (s₀["_3"]!!))⟧)

lemma block_5025993337720852127_abs_of_concrete {s₀ s₉ : State} :
  Spec block_5025993337720852127_concrete_of_code s₀ s₉ →
  Spec A_block_5025993337720852127 s₀ s₉ := by
  unfold block_5025993337720852127_concrete_of_code A_block_5025993337720852127
