...
import generated.InteropHandler.InteropHandler.Common.if_1209118431116190868_gen
...
Abstract spec for the Yul block
    ...
    if iszero(split_expr_4) { revert(0, 0) }

* Guard `split_expr_4` zero  ⇒ the block REVERTS: the result is the initial state with
  its EVM replaced by `evm_revert evm 0 0`, so `s₉.evm.reverted = true`.
* Guard `split_expr_4` nonzero ⇒ the block is a no-op: `s₉ = s₀`.

Self-contained: does not mention `if_1209118431116190868_concrete_of_code`, so the bridge
