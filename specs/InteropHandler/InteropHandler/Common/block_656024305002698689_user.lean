...
import generated.InteropHandler.InteropHandler.Common.block_656024305002698689_gen
...
Abstract spec for the Yul block
    ...
    {
      let _1 := add(var_interopBundle_mpos, 160)
      let split_expr_4 := mload(_1)
      let length := mload(split_expr_4)
      let var_i := 0
    }
...
Self-contained: does not mention `block_656024305002698689_concrete_of_code`.
-/
def A_block_656024305002698689 (s₀ s₉ : State) : Prop :=
  s₉ = ((((s₀⟦"_1" ↦ (s₀["var_interopBundle_mpos"]!!) + (160 : UInt256)⟧)⟦"split_expr_4" ↦ (s₀.evm.mload ((s₀["var_interopBundle_mpos"]!!) + (160 : UInt256)))⟧)⟦"length" ↦ (s₀.evm.mload (s₀.evm.mload ((s₀["var_interopBundle_mpos"]!!) + (160 : UInt256))))⟧)⟦"var_i" ↦ (0 : UInt256)⟧)

lemma block_656024305002698689_abs_of_concrete {s₀ s₉ : State} :
  Spec block_656024305002698689_concrete_of_code s₀ s₉ →
  Spec A_block_656024305002698689 s₀ s₉ := by
  unfold block_656024305002698689_concrete_of_code A_block_656024305002698689
