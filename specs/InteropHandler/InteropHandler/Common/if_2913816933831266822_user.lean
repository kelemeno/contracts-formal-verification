...
Abstract spec for the Yul block
    ...
    if slt(split_expr_3, 160) { revert(0, 0) }

`slt` yields `fromBool (UInt256.slt split_expr_3 160)` (SIGNED less-than) and Yul's `if` fires on a
...
Self-contained: does not mention `if_2913816933831266822_concrete_of_code`.
-/
def A_if_2913816933831266822 (s₀ s₉ : State) : Prop :=
  (UInt256.slt (s₀["split_expr_3"]!!) (160 : UInt256) = true →
      s₉ = s₀🇪⟦s₀.evm.evm_revert 0 0⟧ ∧ s₉.evm.reverted = true)
  ∧ (UInt256.slt (s₀["split_expr_3"]!!) (160 : UInt256) = false → s₉ = s₀)
...
lemma if_2913816933831266822_abs_of_concrete {s₀ s₉ : State} :
  ...
  unfold if_2913816933831266822_concrete_of_code A_if_2913816933831266822
