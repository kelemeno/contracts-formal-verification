...
import generated.InteropHandler.InteropHandler.Common.if_1263889886548336254_gen
...
`v = 0`, so the body runs precisely when `split_expr_3` is zero.  On that branch every
...
Self-contained: does not mention `if_1263889886548336254_concrete_of_code`.
-/
def A_if_1263889886548336254 (s₀ s₉ : State) : Prop :=
  ((s₀["split_expr_3"]!!) = 0 → s₉ = ((s₀⟦"var_value" ↦ (s₀["result_3"]!!)⟧)⟦"var_result" ↦ (s₀["var_result"]!!) + (2 : UInt256)⟧))
  ∧ (¬ ((s₀["split_expr_3"]!!) = 0) → s₉ = s₀)

lemma if_1263889886548336254_abs_of_concrete {s₀ s₉ : State} :
  Spec if_1263889886548336254_concrete_of_code s₀ s₉ →
  Spec A_if_1263889886548336254 s₀ s₉ := by
  unfold if_1263889886548336254_concrete_of_code A_if_1263889886548336254
