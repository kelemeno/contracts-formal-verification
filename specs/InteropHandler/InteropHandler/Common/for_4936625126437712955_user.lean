...
def ACond_for_4936625126437712955 (s₀ : State) : Literal :=
  fromBool (s₀["src"]!! < s₀["srcEnd"]!!)

/-- Loop post: `src := add(src, 32)`. -/
def APost_for_4936625126437712955 (s₀ s₉ : State) : Prop :=
...
and post. -/
def AFor_for_4936625126437712955 (s₀ s₉ : State) : Prop :=
...
not build on it.  `for_4936625126437712955_concrete_of_body_abs` is still `sorry`, so nothing
claims this matches the code, and the seven lemmas below take `ABody` only as an unused
hypothesis — they hold for any `ABody` whatever, which is why they are already proven.

The real form is large: this body has TWO revert guards and six intermediate `let`s, so
the closed form nests two `Spec A_if_...` existentials around chains that re-expand at
every step.  Transcribing it is the remaining work here. -/
def ABody_for_4936625126437712955 (s₀ s₉ : State) : Prop :=
...
lemma for_4936625126437712955_cond_abs_of_code {s₀ fuel} : eval fuel for_4936625126437712955_cond (s₀) = (s₀, ACond_for_4936625126437712955 (s₀)) := by
  ...
  simp [for_4936625126437712955_cond, evalArgs, head', reverse', Var', Lit', PrimCall', evalPrimCall]
  rw [EVMLt']
  simp [fromBool]
...
lemma for_4936625126437712955_concrete_of_post_abs {s₀ s₉ : State} :
  ...
  unfold for_4936625126437712955_post_concrete_of_code APost_for_4936625126437712955
...
lemma AZero_for_4936625126437712955 : ∀ s₀, isOk s₀ → ACond_for_4936625126437712955 (👌 s₀) = 0 → AFor_for_4936625126437712955 s₀ s₀ := by
  intro s₀ hok hcond
  unfold AFor_for_4936625126437712955 ACond_for_4936625126437712955 at *
  intro evm store hs
  subst hs
  intro hlt
...
lemma AOk_for_4936625126437712955 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isOk s₂ → ¬ ❓ s₅ → ¬ ACond_for_4936625126437712955 s₀ = 0 → ABody_for_4936625126437712955 s₀ s₂ → APost_for_4936625126437712955 s₂ s₄ → Spec AFor_for_4936625126437712955 s₄ s₅ → AFor_for_4936625126437712955 s₀ s₅ := by
  ...
  · have h4 : s₄ = (Ok e2 st2)⟦"src" ↦ ((Ok e2 st2)["src"]!! + 32)⟧ := hpost e2 st2 rfl
    ...
    exact Spec_ok_unfold (P := AFor_for_4936625126437712955) (s := s₄) (s' := s₅) hok4 h5 hspec
  · exact absurd h2 (by simp [isOk])
  · exact absurd h2 (by simp [isOk])

lemma AContinue_for_4936625126437712955 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isContinue s₂ → ¬ ACond_for_4936625126437712955 s₀ = 0 → ABody_for_4936625126437712955 s₀ s₂ → Spec APost_for_4936625126437712955 (🧟s₂) s₄ → Spec AFor_for_4936625126437712955 s₄ s₅ → AFor_for_4936625126437712955 s₀ s₅ := by
  ...
  · exact Spec_ok_unfold (P := AFor_for_4936625126437712955) (s := Ok e4 st4) (s' := s₅) (by simp [isOk]) h5 hspec evm store hs
...
lemma ALeave_for_4936625126437712955 : ∀ s₀ s₂, isOk s₀ → isLeave s₂ → ¬ ACond_for_4936625126437712955 s₀ = 0 → ABody_for_4936625126437712955 s₀ s₂ → AFor_for_4936625126437712955 s₀ s₂ := by
...
lemma ABreak_for_4936625126437712955 : ∀ s₀ s₂, isOk s₀ → isBreak s₂ → ¬ ACond_for_4936625126437712955 s₀ = 0 → ABody_for_4936625126437712955 s₀ s₂ → AFor_for_4936625126437712955 s₀ (🧟s₂) := by
  sorry
