...
import generated.InteropHandler.InteropHandler.Common.block_2448294379448399750_gen
...
Self-contained: does not mention `block_2448294379448399750_concrete_of_code`.
-/
def A_block_2448294379448399750 (s₀ s₉ : State) : Prop :=
  s₉ = (((s₀⟦"sum_1" ↦ (65545 : UInt256)⟧)⟦"_5" ↦ (0 : UInt256)⟧)⟦"_6" ↦ (s₀.evm.mload (s₀["var_proof_mpos"]!!))⟧)

lemma block_2448294379448399750_abs_of_concrete {s₀ s₉ : State} :
  Spec block_2448294379448399750_concrete_of_code s₀ s₉ →
  Spec A_block_2448294379448399750 s₀ s₉ := by
  unfold block_2448294379448399750_concrete_of_code A_block_2448294379448399750
