...
import generated.InteropHandler.InteropHandler.Common.block_327334608804541385_gen
...
Abstract spec for the Yul block
    ...
    {
      mstore(split_expr_7, split_expr_9)
      let split_expr_10 := add(value1, 96)
      let memberValue0 := mload(split_expr_10)
      let split_expr_11 := add(tail_1, 96)
      mstore(split_expr_11, 160)
    }
...
Self-contained: does not mention `block_327334608804541385_concrete_of_code`.
-/
def A_block_327334608804541385 (s₀ s₉ : State) : Prop :=
  s₉ = ((((s₀⟦"split_expr_10" ↦ (s₀["value1"]!!) + (96 : UInt256)⟧)⟦"memberValue0" ↦ ((s₀.evm.mstore (s₀["split_expr_7"]!!) (s₀["split_expr_9"]!!)).mload ((s₀["value1"]!!) + (96 : UInt256)))⟧)⟦"split_expr_11" ↦ (s₀["tail_1"]!!) + (96 : UInt256)⟧)🇪⟦((s₀.evm.mstore (s₀["split_expr_7"]!!) (s₀["split_expr_9"]!!)).mstore ((s₀["tail_1"]!!) + (96 : UInt256)) (160 : UInt256))⟧)

lemma block_327334608804541385_abs_of_concrete {s₀ s₉ : State} :
  Spec block_327334608804541385_concrete_of_code s₀ s₉ →
  Spec A_block_327334608804541385 s₀ s₉ := by
  unfold block_327334608804541385_concrete_of_code A_block_327334608804541385
