...
`v = 0`, so the body runs precisely when `split_expr_0` is zero.  On that branch every
...
Self-contained: does not mention `if_560894939903349110_concrete_of_code`.
-/
def A_if_560894939903349110 (s₀ s₉ : State) : Prop :=
  ((s₀["split_expr_0"]!!) = 0 → s₉ = ((s₀⟦"var_value" ↦ (s₀["result"]!!)⟧)⟦"var_result" ↦ (16 : UInt256)⟧))
...
lemma if_560894939903349110_abs_of_concrete {s₀ s₉ : State} :
  ...
  unfold if_560894939903349110_concrete_of_code A_if_560894939903349110
