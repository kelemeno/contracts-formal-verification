...
import generated.InteropHandler.InteropHandler.Common.block_3266888615877513900_gen
...
Abstract spec for the Yul block
    ...
    {
      mstore(split_expr_25, split_expr_27)
      let split_expr_28 := add(_3, 192)
      let offset_1 := mload(split_expr_28)
    }
...
Self-contained: does not mention `block_3266888615877513900_concrete_of_code`.
-/
def A_block_3266888615877513900 (s₀ s₉ : State) : Prop :=
  s₉ = (((s₀⟦"split_expr_28" ↦ (s₀["_3"]!!) + (192 : UInt256)⟧)⟦"offset_1" ↦ ((s₀.evm.mstore (s₀["split_expr_25"]!!) (s₀["split_expr_27"]!!)).mload ((s₀["_3"]!!) + (192 : UInt256)))⟧)🇪⟦(s₀.evm.mstore (s₀["split_expr_25"]!!) (s₀["split_expr_27"]!!))⟧)

lemma block_3266888615877513900_abs_of_concrete {s₀ s₉ : State} :
  Spec block_3266888615877513900_concrete_of_code s₀ s₉ →
  Spec A_block_3266888615877513900 s₀ s₉ := by
  unfold block_3266888615877513900_concrete_of_code A_block_3266888615877513900
