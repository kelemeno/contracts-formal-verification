...
def ACond_for_2051255765951039157 (s₀ : State) : Literal :=
  fromBool (s₀["i"]!! < s₀["length"]!!)

/-- Loop post: `i := add(i, 1)`. -/
def APost_for_2051255765951039157 (s₀ s₉ : State) : Prop :=
...
and post. -/
def AFor_for_2051255765951039157 (s₀ s₉ : State) : Prop :=
...
/-- Loop body: copy one word from `srcPtr` to `pos`, advancing both cursors.
    ...
    let split_expr_0 := sload(srcPtr);  mstore(pos, split_expr_0)
    pos := add(pos, 32);  srcPtr := add(srcPtr, 1) -/
def ABody_for_2051255765951039157 (s₀ s₉ : State) : Prop :=
  ∀ evm store, s₀ = Ok evm store →
    let s₁ := (Ok evm store)⟦"split_expr_0" ↦
                EVMState.sload (Ok evm store).evm ((Ok evm store)["srcPtr"]!!)⟧
    let s₂ := s₁🇪⟦EVMState.mstore (Ok evm store).evm (s₁["pos"]!!) (s₁["split_expr_0"]!!)⟧
    let s₃ := s₂⟦"pos" ↦ (s₂["pos"]!! + 32)⟧
    s₉ = s₃⟦"srcPtr" ↦ (s₃["srcPtr"]!! + 1)⟧
...
lemma for_2051255765951039157_concrete_of_post_abs {s₀ s₉ : State} :
  ...
  unfold for_2051255765951039157_post_concrete_of_code APost_for_2051255765951039157
...
lemma AZero_for_2051255765951039157 : ∀ s₀, isOk s₀ → ACond_for_2051255765951039157 (👌 s₀) = 0 → AFor_for_2051255765951039157 s₀ s₀ := by
  intro s₀ hok hcond
  unfold AFor_for_2051255765951039157 ACond_for_2051255765951039157 at *
  intro evm store hs
  subst hs
  intro hlt
...
lemma AOk_for_2051255765951039157 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isOk s₂ → ¬ ❓ s₅ → ¬ ACond_for_2051255765951039157 s₀ = 0 → ABody_for_2051255765951039157 s₀ s₂ → APost_for_2051255765951039157 s₂ s₄ → Spec AFor_for_2051255765951039157 s₄ s₅ → AFor_for_2051255765951039157 s₀ s₅ := by
  ...
  · have h4 : s₄ = (Ok e2 st2)⟦"i" ↦ ((Ok e2 st2)["i"]!! + 1)⟧ := hpost e2 st2 rfl
    ...
    exact Spec_ok_unfold (P := AFor_for_2051255765951039157) (s := s₄) (s' := s₅) hok4 h5 hspec
...
lemma AContinue_for_2051255765951039157 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isContinue s₂ → ¬ ACond_for_2051255765951039157 s₀ = 0 → ABody_for_2051255765951039157 s₀ s₂ → Spec APost_for_2051255765951039157 (🧟s₂) s₄ → Spec AFor_for_2051255765951039157 s₄ s₅ → AFor_for_2051255765951039157 s₀ s₅ := by
  ...
  · exact Spec_ok_unfold (P := AFor_for_2051255765951039157) (s := Ok e4 st4) (s' := s₅) (by simp [isOk]) h5 hspec evm store hs
...
lemma for_2051255765951039157_concrete_of_body_abs {s₀ s₉ : State} :
  ...
  unfold for_2051255765951039157_body_concrete_of_code ABody_for_2051255765951039157
  rcases s₀ with ⟨evm, store⟩ | _ | _ <;> [skip; aesop_spec; aesop_spec]
  apply spec_eq
  intro _hne hc
  intro evmA storeA hok
