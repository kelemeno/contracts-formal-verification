...
import generated.L1Bridgehub.L1Bridgehub.finalize_allocation_gen
...
def finalize_allocation_if_state (s₀ : State) (memPtr size : Literal) : State :=
  ...
  let s := s⟦"newFreePtr" ↦ memPtr + finalize_allocation_rounded size⟧
  let s := s⟦"split_expr_3" ↦
    (if 18446744073709551615 < memPtr + finalize_allocation_rounded size then 1 else 0)⟧
  s⟦"split_expr_4" ↦
    (if memPtr + finalize_allocation_rounded size < memPtr then 1 else 0)⟧
...
lemma finalize_allocation_abs_of_concrete {s₀ s₉ : State} {memPtr size} :
  ...
  have hss_not_outOfFuel : ¬ ❓ ss := by
    ...
    · simp at hss_outOfFuel
  clr_spec at hif
  have hsafe' : ¬ finalize_allocation_overflow
      (finalize_allocation_if_state (Ok evm varstore) memPtr size) := hsafe
  have hss_eq := hif (by
    revert hsafe'
    unfold finalize_allocation_overflow finalize_allocation_if_state
      finalize_allocation_rounded
    repeat rw [multifill_cons]
    repeat rw [multifill_nil]
    repeat rw [multifill_nil']
    repeat first
      | rw [lookup_insert_of_ne (by decide)]
      | rw [lookup_insert' (by aesop_spec)]
    simp [Bool.toUInt256, fromBool])
  rw [hss_eq] at hfinal
