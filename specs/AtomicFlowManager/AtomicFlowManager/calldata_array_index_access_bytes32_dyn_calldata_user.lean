...
def A_calldata_array_index_access_bytes32_dyn_calldata (addr : Identifier) (base_ref length index : Literal) (s₀ s₉ : State) : Prop := calldata_array_index_access_bytes32_dyn_calldata_concrete_of_code.1 addr base_ref length index s₀ s₉
...
lemma calldata_array_index_access_bytes32_dyn_calldata_abs_of_concrete {s₀ s₉ : State} {addr base_ref length index} :
  ...
  intro h
  simpa [A_calldata_array_index_access_bytes32_dyn_calldata] using h
