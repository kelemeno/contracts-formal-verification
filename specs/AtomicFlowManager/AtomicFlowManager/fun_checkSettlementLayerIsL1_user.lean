...
def A_fun_checkSettlementLayerIsL1  (var_settlementLayerChainId : Literal) (s₀ s₉ : State) : Prop := fun_checkSettlementLayerIsL1_concrete_of_code.1 var_settlementLayerChainId s₀ s₉
...
lemma fun_checkSettlementLayerIsL1_abs_of_concrete {s₀ s₉ : State} { var_settlementLayerChainId} :
  ...
  intro h
  simpa [A_fun_checkSettlementLayerIsL1] using h
