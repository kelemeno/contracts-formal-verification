...
def tailLoadLenBytes32 (s : State) : State :=
...
def tailBoundBytes32 (s : State) : State :=
...
```
    function access_calldata_tail_array_bytes32_dyn_calldata(base_ref, ptr_to_tail)
        -> addr, length
```
...
def A_access_calldata_tail_array_bytes32_dyn_calldata
  ...
  ∃ s₁, Spec AtomicFlowManager.Common.A_block_6743186873342481897
    ...
    ∃ s₂, Spec AtomicFlowManager.Common.A_block_5731116343986243113 s₁ s₂ ∧
      ∃ s₃, Spec AtomicFlowManager.Common.A_if_1209118431116190868 s₂ s₃ ∧
        ∃ s₄, Spec AtomicFlowManager.Common.A_if_6747681429752853338 (tailLoadLenBytes32 s₃) s₄ ∧
          ∃ s₅, Spec AtomicFlowManager.Common.A_if_8835011984658778953 (tailBoundBytes32 s₄) s₅ ∧
...
lemma access_calldata_tail_array_bytes32_dyn_calldata_abs_of_concrete {s₀ s₉ : State} {addr length base_ref ptr_to_tail} :
  ...
  unfold access_calldata_tail_array_bytes32_dyn_calldata_concrete_of_code
    A_access_calldata_tail_array_bytes32_dyn_calldata tailLoadLenBytes32 tailBoundBytes32
...
decides this. -/
lemma access_calldata_tail_array_bytes32_dyn_calldata_isOk
    {addr length : Identifier} {base_ref ptr_to_tail : Literal} {s₀ s₉ : State} (hnf : ¬ ❓ s₉)
    (h : A_access_calldata_tail_array_bytes32_dyn_calldata addr length base_ref ptr_to_tail s₀ s₉) :
    isOk s₉ := by
...
lemma access_calldata_tail_array_bytes32_dyn_calldata_not_break
    {addr length : Identifier} {base_ref ptr_to_tail : Literal} {s₀ s₉ : State} (hnf : ¬ ❓ s₉)
    (h : A_access_calldata_tail_array_bytes32_dyn_calldata addr length base_ref ptr_to_tail s₀ s₉) :
    ¬ isBreak s₉ :=
  fun hb => not_isOk_of_isBreak hb (access_calldata_tail_array_bytes32_dyn_calldata_isOk hnf h)
