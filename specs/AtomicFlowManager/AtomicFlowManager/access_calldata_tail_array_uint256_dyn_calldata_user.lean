...
def A_access_calldata_tail_array_uint256_dyn_calldata (addr length : Identifier) (base_ref ptr_to_tail : Literal) (s₀ s₉ : State) : Prop := access_calldata_tail_array_uint256_dyn_calldata_concrete_of_code.1 addr length base_ref ptr_to_tail s₀ s₉
...
lemma access_calldata_tail_array_uint256_dyn_calldata_abs_of_concrete {s₀ s₉ : State} {addr length base_ref ptr_to_tail} :
  ...
  intro h
  simpa [A_access_calldata_tail_array_uint256_dyn_calldata] using h
