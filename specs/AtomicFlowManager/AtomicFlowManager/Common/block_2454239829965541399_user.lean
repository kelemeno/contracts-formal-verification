...
import generated.AtomicFlowManager.AtomicFlowManager.Common.block_2454239829965541399_gen
...
def A_block_2454239829965541399 (s₀ s₉ : State) : Prop :=
  let a := s₀⟦"split_expr_3" ↦ s₀["expr_2619_mpos"]!! + 96⟧
  let m1 := a🇪⟦Clear.EVMState.mstore s₀.evm (a["split_expr_3"]!!) (a["_3"]!!)⟧
  let m2 := m1🇪⟦Clear.EVMState.mstore m1.evm (m1["expr_2619_mpos"]!!) 96⟧
  ∃ s, Spec (A_finalize_allocation (m2["expr_2619_mpos"]!!) 128) m2 s ∧
    s₉ = s⟦"split_expr_4" ↦ Clear.EVMState.mload s.evm (s["expr_2619_mpos"]!!)⟧

lemma block_2454239829965541399_abs_of_concrete {s₀ s₉ : State} :
  Spec block_2454239829965541399_concrete_of_code s₀ s₉ →
  Spec A_block_2454239829965541399 s₀ s₉ := by
  unfold block_2454239829965541399_concrete_of_code A_block_2454239829965541399
