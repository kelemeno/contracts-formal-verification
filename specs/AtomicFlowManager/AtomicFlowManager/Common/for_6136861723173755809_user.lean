...
def ACond_for_6136861723173755809 (s₀ : State) : Literal :=
  fromBool (s₀["src"]!! < s₀["srcEnd"]!!)

/-- Loop post: `src := add(src, 32)`. -/
def APost_for_6136861723173755809 (s₀ s₉ : State) : Prop :=
...
and post. -/
def AFor_for_6136861723173755809 (s₀ s₉ : State) : Prop :=
...
/-- Loop body: copy one calldata word to memory and advance the destination.
    ...
    let value := 0;  value := calldataload(src);  mstore(dst, value);  dst := add(dst, 32)

The `let value := 0` and the assignment collapse to a single insert, whose argument still
reads `src` through the zero-initialised store — mirrored below rather than simplified. -/
def ABody_for_6136861723173755809 (s₀ s₉ : State) : Prop :=
  ∀ evm store, s₀ = Ok evm store →
    let s₁ := (Ok evm store)⟦"value" ↦ 0⟧
    let s₂ := (Ok evm store)⟦"value" ↦ EVMState.calldataload (Ok evm store).evm (s₁["src"]!!)⟧
    let s₃ := s₂🇪⟦EVMState.mstore (Ok evm store).evm (s₂["dst"]!!) (s₂["value"]!!)⟧
    s₉ = s₃⟦"dst" ↦ (s₃["dst"]!! + 32)⟧
...
lemma for_6136861723173755809_concrete_of_post_abs {s₀ s₉ : State} :
  ...
  unfold for_6136861723173755809_post_concrete_of_code APost_for_6136861723173755809
...
lemma AZero_for_6136861723173755809 : ∀ s₀, isOk s₀ → ACond_for_6136861723173755809 (👌 s₀) = 0 → AFor_for_6136861723173755809 s₀ s₀ := by
  intro s₀ hok hcond
  unfold AFor_for_6136861723173755809 ACond_for_6136861723173755809 at *
  intro evm store hs
  subst hs
  intro hlt
...
lemma AOk_for_6136861723173755809 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isOk s₂ → ¬ ❓ s₅ → ¬ ACond_for_6136861723173755809 s₀ = 0 → ABody_for_6136861723173755809 s₀ s₂ → APost_for_6136861723173755809 s₂ s₄ → Spec AFor_for_6136861723173755809 s₄ s₅ → AFor_for_6136861723173755809 s₀ s₅ := by
  ...
  · have h4 : s₄ = (Ok e2 st2)⟦"src" ↦ ((Ok e2 st2)["src"]!! + 32)⟧ := hpost e2 st2 rfl
    ...
    exact Spec_ok_unfold (P := AFor_for_6136861723173755809) (s := s₄) (s' := s₅) hok4 h5 hspec
...
lemma AContinue_for_6136861723173755809 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isContinue s₂ → ¬ ACond_for_6136861723173755809 s₀ = 0 → ABody_for_6136861723173755809 s₀ s₂ → Spec APost_for_6136861723173755809 (🧟s₂) s₄ → Spec AFor_for_6136861723173755809 s₄ s₅ → AFor_for_6136861723173755809 s₀ s₅ := by
  ...
  · exact Spec_ok_unfold (P := AFor_for_6136861723173755809) (s := Ok e4 st4) (s' := s₅) (by simp [isOk]) h5 hspec evm store hs
...
lemma for_6136861723173755809_concrete_of_body_abs {s₀ s₉ : State} :
  ...
  unfold for_6136861723173755809_body_concrete_of_code ABody_for_6136861723173755809
  rcases s₀ with ⟨evm, store⟩ | _ | _ <;> [skip; aesop_spec; aesop_spec]
  apply spec_eq
  intro _hne hc
  intro evmA storeA hok
