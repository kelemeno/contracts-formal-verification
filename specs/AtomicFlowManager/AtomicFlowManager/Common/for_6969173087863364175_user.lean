...
def ACond_for_6969173087863364175 (s₀ : State) : Literal := fromBool (s₀["var_i"]!! < s₀["length"]!!)
def APost_for_6969173087863364175 (s₀ s₉ : State) : Prop := for_6969173087863364175_post_concrete_of_code.1 s₀ s₉
def ABody_for_6969173087863364175 (s₀ s₉ : State) : Prop := for_6969173087863364175_body_concrete_of_code.1 s₀ s₉
def AFor_for_6969173087863364175 (s₀ s₉ : State) : Prop := True
...
lemma for_6969173087863364175_cond_abs_of_code {s₀ fuel} : eval fuel for_6969173087863364175_cond (s₀) = (s₀, ACond_for_6969173087863364175 (s₀)) := by
  ...
  simp [for_6969173087863364175_cond, evalArgs, head', reverse', Var', Lit', PrimCall', evalPrimCall]
  rw [EVMLt']
  simp [fromBool]
...
lemma for_6969173087863364175_concrete_of_post_abs {s₀ s₉ : State} :
  ...
  intro h
  simpa [APost_for_6969173087863364175] using h
...
lemma for_6969173087863364175_concrete_of_body_abs {s₀ s₉ : State} :
  ...
  intro h
  simpa [ABody_for_6969173087863364175] using h

lemma AZero_for_6969173087863364175 : ∀ s₀, isOk s₀ → ACond_for_6969173087863364175 (👌 s₀) = 0 → AFor_for_6969173087863364175 s₀ s₀ := by intros; trivial
...
:= by intros; trivial
lemma AContinue_for_6969173087863364175 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isContinue s₂ → ¬ ACond_for_6969173087863364175 s₀ = 0 → ABody_for_6969173087863364175 s₀ s₂ → Spec APost_for_6969173087863364175 (🧟s₂) s₄ → Spec AFor_for_6969173087863364175 s₄ s₅ → AFor_for_6969173087863364175 s₀ s₅ := by intros; trivial
lemma ABreak_for_6969173087863364175 : ∀ s₀ s₂, isOk s₀ → isBreak s₂ → ¬ ACond_for_6969173087863364175 s₀ = 0 → ABody_for_6969173087863364175 s₀ s₂ → AFor_for_6969173087863364175 s₀ (🧟s₂) := by intros; trivial
lemma ALeave_for_6969173087863364175 : ∀ s₀ s₂, isOk s₀ → isLeave s₂ → ¬ ACond_for_6969173087863364175 s₀ = 0 → ABody_for_6969173087863364175 s₀ s₂ → AFor_for_6969173087863364175 s₀ s₂ := by intros; trivial
