...
/-- Loop condition: `lt(var_i, var_right)`. -/
def ACond_for_6242390032430749259 (s₀ : State) : Literal :=
  fromBool (s₀["var_i"]!! < s₀["var_right"]!!)

/-- Loop post: `var_i := add(var_i, 1)`. -/
def APost_for_6242390032430749259 (s₀ s₉ : State) : Prop :=
...
and post. -/
def AFor_for_6242390032430749259 (s₀ s₉ : State) : Prop :=
...
/-- Loop body: copy one Merkle-proof word into the in-memory `InteropCall` array.
    ...
    let _2 := calldata_array_index_access_bytes32_dyn_calldata(proof_offset, proof_length, var_i)
...
def ABody_for_6242390032430749259 (s₀ s₉ : State) : Prop :=
  ∃ s, Spec (A_calldata_array_index_access_bytes32_dyn_calldata "_2"
              (s₀["var_proof_4073_offset"]!!) (s₀["var__proof_length"]!!) (s₀["var_i"]!!)) s₀ s ∧
...
lemma for_6242390032430749259_concrete_of_post_abs {s₀ s₉ : State} :
  ...
  unfold for_6242390032430749259_post_concrete_of_code APost_for_6242390032430749259
...
lemma AZero_for_6242390032430749259 : ∀ s₀, isOk s₀ → ACond_for_6242390032430749259 (👌 s₀) = 0 → AFor_for_6242390032430749259 s₀ s₀ := by
  intro s₀ hok hcond
  unfold AFor_for_6242390032430749259 ACond_for_6242390032430749259 at *
  intro evm store hs
  subst hs
  intro hlt
...
lemma AOk_for_6242390032430749259 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isOk s₂ → ¬ ❓ s₅ → ¬ ACond_for_6242390032430749259 s₀ = 0 → ABody_for_6242390032430749259 s₀ s₂ → APost_for_6242390032430749259 s₂ s₄ → Spec AFor_for_6242390032430749259 s₄ s₅ → AFor_for_6242390032430749259 s₀ s₅ := by
  ...
  · have h4 : s₄ = (Ok e2 st2)⟦"var_i" ↦ ((Ok e2 st2)["var_i"]!! + 1)⟧ := hpost e2 st2 rfl
    ...
    exact Spec_ok_unfold (P := AFor_for_6242390032430749259) (s := s₄) (s' := s₅) hok4 h5 hspec
  · exact absurd h2 (by simp [isOk])
  · exact absurd h2 (by simp [isOk])

lemma AContinue_for_6242390032430749259 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isContinue s₂ → ¬ ACond_for_6242390032430749259 s₀ = 0 → ABody_for_6242390032430749259 s₀ s₂ → Spec APost_for_6242390032430749259 (🧟s₂) s₄ → Spec AFor_for_6242390032430749259 s₄ s₅ → AFor_for_6242390032430749259 s₀ s₅ := by
  ...
  · exact Spec_ok_unfold (P := AFor_for_6242390032430749259) (s := Ok e4 st4) (s' := s₅) (by simp [isOk]) h5 hspec evm store hs
...
lemma ALeave_for_6242390032430749259 : ∀ s₀ s₂, isOk s₀ → isLeave s₂ → ¬ ACond_for_6242390032430749259 s₀ = 0 → ABody_for_6242390032430749259 s₀ s₂ → AFor_for_6242390032430749259 s₀ s₂ := by
...
lemma for_6242390032430749259_concrete_of_body_abs {s₀ s₉ : State} :
  ...
  unfold for_6242390032430749259_body_concrete_of_code ABody_for_6242390032430749259
  rcases s₀ with ⟨evm, store⟩ | _ | _ <;> [skip; aesop_spec; aesop_spec]
  apply spec_eq
  intro _hne hc
  exact hc
...
lemma ABreak_for_6242390032430749259 : ∀ s₀ s₂, isOk s₀ → isBreak s₂ → ¬ ACond_for_6242390032430749259 s₀ = 0 → ABody_for_6242390032430749259 s₀ s₂ → AFor_for_6242390032430749259 s₀ (🧟s₂) := by
  ...
  have hsok : isOk s :=
    calldata_array_index_access_bytes32_dyn_calldata_isOk h0 hs_nf
      (Spec_ok_unfold (P := A_calldata_array_index_access_bytes32_dyn_calldata _ _ _ _)
        h0 hs_nf hacc)
  have hssok : isOk ss :=
