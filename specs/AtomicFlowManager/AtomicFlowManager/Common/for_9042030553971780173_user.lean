...
/-- Loop condition: `lt(var_i, expr_576_length)`. -/
def ACond_for_9042030553971780173 (s₀ : State) : Literal :=
  fromBool (s₀["var_i"]!! < s₀["expr_576_length"]!!)

/-- Loop post: `var_i := add(var_i, 1)`. -/
def APost_for_9042030553971780173 (s₀ s₉ : State) : Prop :=
...
thread it through unchanged, and it is sound because `expr_576_length` is untouched by body
and post. -/
def AFor_for_9042030553971780173 (s₀ s₉ : State) : Prop :=
  ∀ evm store, s₉ = Ok evm store → ¬ ((Ok evm store)["var_i"]!! < (Ok evm store)["expr_576_length"]!!)

/-- Loop body: the three blocks in sequence — read `arr[i]`, read `arr[i-1]` and
compare, then revert unless the comparison held. -/
def ABody_for_9042030553971780173 (s₀ s₉ : State) : Prop :=
  ∃ s₁, Spec A_block_932755155890173629 s₀ s₁ ∧
    ∃ s₂, Spec A_block_253019513998627002 s₁ s₂ ∧
...
lemma for_9042030553971780173_concrete_of_post_abs {s₀ s₉ : State} :
  ...
  unfold for_9042030553971780173_post_concrete_of_code APost_for_9042030553971780173
...
lemma AZero_for_9042030553971780173 : ∀ s₀, isOk s₀ → ACond_for_9042030553971780173 (👌 s₀) = 0 → AFor_for_9042030553971780173 s₀ s₀ := by
  intro s₀ hok hcond
  unfold AFor_for_9042030553971780173 ACond_for_9042030553971780173 at *
  intro evm store hs
  subst hs
  intro hlt
...
lemma AOk_for_9042030553971780173 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isOk s₂ → ¬ ❓ s₅ → ¬ ACond_for_9042030553971780173 s₀ = 0 → ABody_for_9042030553971780173 s₀ s₂ → APost_for_9042030553971780173 s₂ s₄ → Spec AFor_for_9042030553971780173 s₄ s₅ → AFor_for_9042030553971780173 s₀ s₅ := by
  ...
  · have h4 : s₄ = (Ok e2 st2)⟦"var_i" ↦ ((Ok e2 st2)["var_i"]!! + 1)⟧ := hpost e2 st2 rfl
    ...
    exact Spec_ok_unfold (P := AFor_for_9042030553971780173) (s := s₄) (s' := s₅) hok4 h5 hspec
...
lemma AContinue_for_9042030553971780173 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isContinue s₂ → ¬ ACond_for_9042030553971780173 s₀ = 0 → ABody_for_9042030553971780173 s₀ s₂ → Spec APost_for_9042030553971780173 (🧟s₂) s₄ → Spec AFor_for_9042030553971780173 s₄ s₅ → AFor_for_9042030553971780173 s₀ s₅ := by
  ...
  · exact Spec_ok_unfold (P := AFor_for_9042030553971780173) (s := Ok e4 st4) (s' := s₅) (by simp [isOk]) h5 hspec evm store hs
...
lemma for_9042030553971780173_concrete_of_body_abs {s₀ s₉ : State} :
  ...
  unfold for_9042030553971780173_body_concrete_of_code ABody_for_9042030553971780173
  rcases s₀ with ⟨evm, store⟩ | _ | _ <;> [skip; aesop_spec; aesop_spec]
  apply spec_eq
  intro _hne hc
...
lemma ABreak_for_9042030553971780173 : ∀ s₀ s₂, isOk s₀ → isBreak s₂ → ¬ ACond_for_9042030553971780173 s₀ = 0 → ABody_for_9042030553971780173 s₀ s₂ → AFor_for_9042030553971780173 s₀ (🧟s₂) := by
  ...
  have hs1 : isOk s₁ := block_932755155890173629_isOk h0 h1nf (Spec_ok_unfold h0 h1nf hb₁)
  have hsm : isOk sm := block_253019513998627002_isOk hs1 hmnf (Spec_ok_unfold hs1 hmnf hb₂)
