...
def ACond_for_5484791876761743662 (s₀ : State) : Literal := fromBool (s₀["var_i"]!! < s₀["expr_643_length"]!!)
def APost_for_5484791876761743662 (s₀ s₉ : State) : Prop := for_5484791876761743662_post_concrete_of_code.1 s₀ s₉
def ABody_for_5484791876761743662 (s₀ s₉ : State) : Prop := for_5484791876761743662_body_concrete_of_code.1 s₀ s₉
def AFor_for_5484791876761743662 (s₀ s₉ : State) : Prop := True
...
lemma for_5484791876761743662_concrete_of_post_abs {s₀ s₉ : State} :
  ...
  intro h
  simpa [APost_for_5484791876761743662] using h
...
lemma for_5484791876761743662_concrete_of_body_abs {s₀ s₉ : State} :
  ...
  intro h
  simpa [ABody_for_5484791876761743662] using h

lemma AZero_for_5484791876761743662 : ∀ s₀, isOk s₀ → ACond_for_5484791876761743662 (👌 s₀) = 0 → AFor_for_5484791876761743662 s₀ s₀ := by intros; trivial
...
:= by intros; trivial
lemma AContinue_for_5484791876761743662 : ∀ s₀ s₂ s₄ s₅, isOk s₀ → isContinue s₂ → ¬ ACond_for_5484791876761743662 s₀ = 0 → ABody_for_5484791876761743662 s₀ s₂ → Spec APost_for_5484791876761743662 (🧟s₂) s₄ → Spec AFor_for_5484791876761743662 s₄ s₅ → AFor_for_5484791876761743662 s₀ s₅ := by intros; trivial
lemma ABreak_for_5484791876761743662 : ∀ s₀ s₂, isOk s₀ → isBreak s₂ → ¬ ACond_for_5484791876761743662 s₀ = 0 → ABody_for_5484791876761743662 s₀ s₂ → AFor_for_5484791876761743662 s₀ (🧟s₂) := by intros; trivial
lemma ALeave_for_5484791876761743662 : ∀ s₀ s₂, isOk s₀ → isLeave s₂ → ¬ ACond_for_5484791876761743662 s₀ = 0 → ABody_for_5484791876761743662 s₀ s₂ → AFor_for_5484791876761743662 s₀ s₂ := by intros; trivial
