...
lemma List.exists_append_singleton_of_nonempty {α : Type} {xs : List α}
  ...
  · norm_num at h₁
    ...
    exact ⟨init.reverse, last, List.reverse_inj.mp hrev⟩
...
open Lean Parser Elab Tactic in
elab "post_abstraction_refuel" fuel:ident hfuel:ident hjump:ident s:elimTarget : tactic => do
  evalTactic <| ← `(tactic|
    rcases $fuel:ident with _ | $fuel;
