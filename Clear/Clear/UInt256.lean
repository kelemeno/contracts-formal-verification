...
instance : NatCast UInt256 := ⟨fun n => Fin.ofNat UInt256.size n⟩

abbrev Nat.toUInt256 : ℕ → UInt256 := fun n => Fin.ofNat UInt256.size n
...
private def toBytes' : ℕ → List UInt8
  | 0 => []
  | n@(.succ n') =>
    let byte : UInt8 := UInt8.ofNat (n % UInt8.size)
...
private lemma extend_bytes_zero : fromBytes' (bs ++ List.replicate n 0) = fromBytes' bs := by
  ...
  | nil =>
    ...
    | succ _ ih => simp [List.replicate, fromBytes', UInt8.toNat_zero, ih]
...
private lemma fromBytes'_toBytes' {x : ℕ} : fromBytes' (toBytes' x) = x := by
  ...
  | .succ n =>
    ...
    have hbyte : (UInt8.ofNat (n.succ % UInt8.size)).toNat = n.succ % UInt8.size := by
      show (OfNat.ofNat (n.succ % UInt8.size) : UInt8).toNat = n.succ % UInt8.size
      rw [UInt8.toNat_ofNat]
      exact Nat.mod_eq_of_lt (Nat.mod_lt _ (by simp [UInt8.size]))
    have hdiv : n.succ / UInt8.size ≤ n :=
      Nat.lt_succ_iff.mp (Nat.div_lt_self (Nat.zero_lt_succ n) (by decide : 1 < UInt8.size))
    rw [fromBytes'_toBytes', hbyte]
    have hdm : UInt8.size * (n.succ / UInt8.size) + n.succ % UInt8.size = n.succ :=
      Nat.div_add_mod n.succ UInt8.size
    simp [UInt8.size] at hdm
    linarith
