...
proved theorems over its blocks, not about a verified specification of the function.
  ...
  **Why `fun_verifyBundle`'s bridge is blocked is now known precisely, and it is not effort.**  Of its
  ...
  |---|---|---|
  | ~~4~~ **0** | ~~**large shift constant**~~ — **THIS ROW WAS ENTIRELY WRONG.**  All four blocks filed under it have since been specced by hand (2026-08-11): the shift is carried symbolically by `EVMShl'` and never evaluated, in both the two-implication (if-block) and closed-form (plain block) patterns.  See the corrected gap-2 entry in `AGENTS.md`. | *(none)* |
